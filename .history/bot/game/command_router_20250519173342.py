...
if TYPE_CHECKING:
    ...
    from bot.game.models.party import Party # <--- РАСКОММЕНТИРОВАНО: Needed for "Party" type hint
...
class CommandRouter:
    ...
    def __init__(
        ...
        # self._dialogue_manager = dialogue_manager # Assuming dialogue_manager might be optional
    ...
    async def route(self, message: Message) -> None:
        ...
        context: Dict[str, Any] = {
            ...
            'event_stage_processor': self._event_stage_processor, # Correctly added to context
            # TODO: Add other optional managers like self._dialogue_manager to context
        ...
        if subcommand == "create":
            ...
            try:
             ...
             if subcommand_args:
                  ...
                  # if not target_char and char_id_or_name_to_find and not is_uuid_format(char_id_or_name_to_find): # Вам нужна is_uuid_format функция
    ...
    async def handle_status(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
        ...
        try:
            # FIX: Calling get_character_sheet_embed as intended for the full sheet view.
    ...
    async def handle_party(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
         ...
         if player_char is None and subcommand not in ["help", "info"]: # Commands that might not need player char
         ...
         if subcommand == "create":
              ...
              # Get player's current party using PartyManager.get_party_by_member_id (needs guild_id)
              player_current_party = await party_manager.get_party_by_member_id(player_char_id, guild_id)
              if player_current_party:
                   await send_callback(f"❌ Вы уже состоите в партии (ID `{getattr(player_current_party, 'id', 'N/A')}`). Сначала покиньте ее (`{self._command_prefix}party leave`).")
                   return
              ...
              try:
                   ...
                   if new_party_id:
                        ...
                        print(f"CommandRouter: party_manager.create_party returned None for user {author_id} (char {player_char_id}) in guild {guild_id}. Check manager logs.")
              ...
              if player_current_party:
                   ...
                   if getattr(player_current_party, 'id', None) == getattr(target_party, 'id', None):
                        ...
                        await send_callback(f"❌ Вы уже состоите в другой партии (ID `{getattr(player_current_party, 'id', 'N/A')}`). Сначала покиньте ее (`{self._command_prefix}party leave`).")
                   return

              # Call PartyActionProcessor to handle joining logic
              try:
                   # PartyActionProcessor.process_join_party needs character_id (str), party_id (str), context (Dict)
             ...
             try:
                  ...
                  if leave_successful:
                       ...
                       # Processor should send reason for failure
             ...
             if subcommand_args:
                  ...
                  if player_char_id is None:
                       await send_callback(f"❌ У вас нет персонажа или вы не состоите в партии. Укажите ID партии для просмотра (`{self._command_prefix}party info <ID партии>`).") # Added prefix to usage hint
                       return
                  ...
                  if not target_party:
                       await send_callback(f"❌ Вы не состоите в партии. Укажите ID партии для просмотра (`{self._command_prefix}party info <ID партии>`).") # Added prefix to usage hint
                       return
             ...
             if party_view_service and hasattr(party_view_service, 'get_party_info_embed'): # Check manager and method
                  try:
                      ...
                      if party_embed: # Check if method returned something usable
                           ...
                           await send_callback(f"❌ Не удалось сгенерировать информацию для партии **{getattr(target_party, 'name', 'N/A')}**.")
                  ...
                  if isinstance(member_ids, list) and member_ids: # Ensure member_ids is list and not empty
                       ...
                       for member_id in member_ids:
                            ...
                            member_names.append(f"`{str(member_id)[:6]}` ({name})") # Display truncated ID + name


                  info_message = f"Информация о партии **{party_name}** (ID: `{party_id}`).\n"
                  info_message += f"Лидер: `{str(leader_id)[:6] if leader_id is not None else 'Нет'}`\n" # Display truncated leader ID safely
    ...
    #               return
    #
    #           char = char_mgr.get_character_by_discord_id(guild_id, author_id_int) # Needs guild_id, int discord ID
    ...
    #           loc_view_service = context.get('location_view_service') # Need this service
    ...
    #     # Needs LocationManager (to find target), CharacterManager (player location),

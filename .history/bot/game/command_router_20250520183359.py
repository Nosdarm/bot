...
class CommandRouter:
    ...
    async def handle_party(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
         ...
         if not args:
             ...
             # İSPRAVLENIE: Safely check if docstring exists and is a string before formatting/sending
             if isinstance(help_message, str):
                  help_message = help_message.format(prefix=self._command_prefix)
                  # Check if formatting resulted in an empty string
                  if not help_message:
                       help_message = f"Описание команды '{context.get('command_keyword', 'party')}' недоступно." # Fallback if docstring is empty after format
             else:
                  # İSPRAVLENIE: Fallback message if docstring is None or not a string
                  help_message = f"Описание команды '{context.get('command_keyword', 'party')}' недоступно."
                  print(f"CommandRouter Warning: docstring is missing or not a string for handle_party ({type(self.handle_party.__doc__)}).") # Log type for debug


             await send_callback(help_message) # Send the resolved help message
             return

         # ... остальная логика handle_party ...

...
class CommandRouter:
    ...
    async def handle_help(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
        ...
        if not args:
            ...
            if handler:
                ...
                if not docstring:
                     docstring = f"Нет описания для этой команды (`{target_command}`)." # Fallback if formatting results in empty string
    ...
    async def handle_character(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
        ...
        if not args:
            ...
            if isinstance(help_message, str):
                 ...
                 help_message = "Описание команды недоступно." # Fallback if docstring is not a string

            await send_callback(help_message) # Show usage if no subcommand
            return
        ...
        if subcommand == "create":
            ...
            try:
                ...
                new_character = await char_manager.create_character(
                    ...
                    name=character_name,
                    # guild_id is passed via **context, but explicitly passing might be clearer
                    guild_id=guild_id, # <--- Pass guild_id explicitly
    ...
    async def handle_inventory(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if inventory_embed: # Check if method returned something usable
                 ...
                 await send_callback(f"❌ Не удалось сгенерировать инвентарь для персонажа **{getattr(target_char, 'name', 'N/A')}**. Проверьте логи.")
    ...
    async def handle_party(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
         ...
         if not args:
             # Если нет аргументов (субкоманды), показать сообщение об использовании
             usage_message = (
                 f"Управляет группами персонажей (пати).\n"
                 f"Использование:\n"
                 f"`{self._command_prefix}party create` - Создать новую партию (вы становитесь лидером).\n"
                 f"`{self._command_prefix}party join <ID партии>` - Присоединиться к существующей партии.\n"
                 f"`{self._command_prefix}party leave` - Покинуть текущую партию.\n"
                 f"`{self._command_prefix}party info [<ID партии>]` - Показать информацию о вашей партии или партии по ID."
             )
             await send_callback(usage_message) # Отправляем явно определенную строку
             return

...
if TYPE_CHECKING:
    ...
    # Добавляем другие модели, если их объекты напрямую обрабатываются или передаются в хендлерах
...
class CommandRouter:
    ...
    async def route(self, message: Message) -> None:
        ...
        context: Dict[str, Any] = {
            ...
            'command_keyword': command_keyword, # The top-level command keyword (e.g. 'character', 'status')
            'command_args': command_args, # Raw arguments list passed to the top-level handler
            ...
            self._event_stage_processor: self._event_stage_processor, # Example: if EventStageProcessor is optional
            # TODO: Add other optional managers like self._dialogue_manager

            # Add SendCallback for the current channel for convenience in deeply nested calls
            # Note: This should only be used for sending direct responses in the *command channel*.
            # For sending messages to other channels (e.g. event channels, combat logs), use the factory.
            'send_to_command_channel': self._send_callback_factory(message.channel.id),

            # TODO: Consider adding player_character to context if many commands need it.
            # This adds a manager call overhead to every command execution, so only do if beneficial.
            # For now, handlers that need the player's character fetch it themselves.
        }
    ...
    async def handle_character(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
        """
        Управляет персонажами (создание, удаление).
        Использование:
        ...
        if subcommand == "create":
            ...
            # Example basic check:
            if len(character_name) > 30: # Max name length
                await send_callback("❌ Имя персонажа слишком длинное.")
                return
            # Add other checks...
            ...
            try:
                ...
                new_character = await char_manager.create_character(
                    ...
                    name=character_name,
                    # guild_id is passed via **context
             ...
             if subcommand_args:
                  ...
                  # If not target_char and char_id_or_name_to_find and not is_uuid_format(char_id_or_name_to_find):
             ...
             except (ValueError, TypeError): pass # Already handled above
             ...
             try:
                 ...
                 deleted_char_id = await char_manager.remove_character(
                     ...
                     # Pass full context, which remove_character uses for cleanup calls
                     # Context includes character object, guild_id, send_callback_factory etc.
                     **context
                 )
                 ...
                 import traceback
                 traceback.print_exc()
                 await send_callback(f"❌ Произошла ошибка при удалении персонажа `{char_id_or_name_to_find}`: {e}")
    ...
    async def handle_status(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
        ...
        try:
            # FIX: Reverted to the expected method name from CharacterViewService
            # CharacterViewService.get_character_stats_embed expects Character object and context
            # CharacterViewService.get_character_sheet_embed might exist in *your* code, but was not in our shared CharacterViewService.
            # Using get_character_stats_embed based on previous iterations. If you intend a fuller sheet,
            # ensure your CharacterViewService has a method like get_character_sheet_embed or modify get_character_stats_embed.
            # For now, I'll call the method that *was* implemented in the shared code.
            sheet_embed = await char_view_service.get_character_stats_embed(target_char, context=context) # <-- CORRECTED CALL
            ...
            if sheet_embed: # Check if method returned something usable (like discord.Embed)
                 ...
                 await send_callback(f"❌ Не удалось сгенерировать лист персонажа **{getattr(target_char, 'name', 'N/A')}**.")
    ...
    # Note: /status now shows inventory as part of the sheet.
    # This separate /inventory command might be used for more detailed inventory management later.
    # For now, it's redundant if /status is implemented correctly and includes inventory.
    ...
    async def handle_inventory(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
        """(DEBUG/TEMP) Показывает инвентарь вашего персонажа. Используйте: `[<ID персонажа>]`"""
        send_callback = context['send_to_command_channel'] # Use the callback from context
        guild_id = context.get('guild_id')
        author_id = context['author_id']
    ...
    async def handle_move(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
        ...
        loc_manager = context.get('location_manager') # Type: Optional["LocationManager"] # Need LocationManager for validation
        ...
        # TODO: Validate target_location_id_arg - check if it's a valid location instance ID for this guild
        # Also, check if it's reachable from the current location!
        # This validation logic should likely be in CharacterActionProcessor or LocationManager.
        # For now, pass the raw argument and let the processor/manager handle validation.
        # If validation is done here, need to call loc_manager.get_location_instance(guild_id, target_location_id_arg)

        try:
            # CharacterActionProcessor.process_move_action expects character_id, target_location_id (str), context
            # Processor is expected to handle validation (location exists, reachable, character is not busy)
            ...
            await char_action_processor.process_move_action(
                ...
                target_location_id=str(target_location_id_arg), # Ensure location ID is string
    ...
    async def handle_party(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
         ...
         if not args:
             await send_callback(self.handle_party.__doc__) # Show usage if no subcommand
             return
         ...
         player_char = char_manager.get_character_by_discord_id(guild_id, author_id_int)
         if not player_char:
             # Player needs a character to interact with parties
             if subcommand != "create": # Except maybe /party create, depending on rules if party exists before leader has char
                  await send_callback(f"❌ У вас еще нет персонажа. Создайте его командой `{self._command_prefix}character create <имя>`.")
                  return
             # Note: If create_party logic requires a character object, you'd need to handle the "create without char" case specifically or disallow it.
             # Assuming create_party requires the *leader* to have a character first. Let's re-check create logic below.
         ...
         if subcommand == "create":
              ...
              if player_char_id is not None and party_manager.get_party_by_member_id(player_char_id, guild_id): # Use get_party_by_member_id with guild_id
                   await send_callback("❌ Вы уже состоите в партии. Сначала покиньте ее (`/party leave`).")
                   return

              # Create the party with the player as leader
              try:
                   # party_manager.create_party(leader_id, member_ids, guild_id, **kwargs)
                   ...
                   new_party_id = await party_manager.create_party(
                       ...
                       guild_id=guild_id, # Pass guild_id
                   ...
                   if new_party_id:
                        ...
                        if char_manager and hasattr(char_manager, 'set_party_id'):
                            await char_manager.set_party_id(
                                guild_id=guild_id, # Pass guild_id to char manager
                                character_id=player_char_id,
                                party_id=new_party_id,
                                **context # Pass context
                            )
                            # No need to send feedback here, set_party_id logs internally
                        else:
                            print(f"CommandRouter Warning: CharacterManager not available or missing set_party_id to update character {player_char_id} after party creation {new_party_id} in guild {guild_id}.")

                        await send_callback(f"🎉 Вы успешно создали новую партию! ID партии: `{new_party_id}`")
                        print(f"CommandRouter: Party {new_party_id} created by {author_id} (char {player_char_id}) in guild {guild_id}.")

                   else:
                        # Manager failed to create party (it should log reason)
                        await send_callback("❌ Не удалось создать партию.")
              ...
              # Check if party exists with PartyManager.get_party (needs guild_id)
              ...
              # Check if player is already in this or another party
              player_current_party = await party_manager.get_party_by_member_id(player_char_id, guild_id) # Needs guild_id
              if player_current_party:
                   if getattr(player_current_party, 'id', None) == getattr(target_party, 'id', None):
                        ...
                        await send_callback(f"❌ Вы уже состоите в другой партии. Сначала покиньте ее (`/party leave`).")
                   return

              # Call PartyActionProcessor to handle joining logic
              try:
                   # process_join_party needs character_id, party_id, context
                   # Processor handles adding member, updating char's party_id, sending feedback.
                   ...
                   # This method returns True/False based on success.
                   if join_successful:
                        print(f"CommandRouter: Join party action processed for char {player_char_id} to party {getattr(target_party, 'id')} in guild {guild_id}.")
                        # Processor sends success message
                   else:
                        # Processor should send reason for failure (e.g. party full)
                        print(f"CommandRouter: Join party action failed in processor for char {player_char_id} to party {getattr(target_party, 'id')} in guild {guild_id}.")
                        # Processor sends error message
             ...
             # Handle /party leave
             if player_char_id is None:
                 await send_callback("❌ У вас нет персонажа или вы не состоите в партии.")
                 return

             # Find the player's current party
             player_current_party = await party_manager.get_party_by_member_id(player_char_id, guild_id) # Needs guild_id
             ...
             try:
                  ...
                  if party_id_to_leave is None:
                       print(f"CommandRouter Error: Player's party object has no ID attribute for char {player_char_id} in guild {guild_id}.")
                       await send_callback("❌ Произошла ошибка: Не удалось получить ID вашей партии.")
                       return

                  # process_leave_party needs character_id, party_id, context
                  ...
                  # PartyActionProcessor should send confirmation/error messages directly.
                  if leave_successful:
                       print(f"CommandRouter: Leave party action processed for char {player_char_id} from party {party_id_to_leave} in guild {guild_id}.")
                       # Processor sends success message
                  else:
             ...
             if subcommand_args:
                  ...
                  party_id_to_view = subcommand_args[0]
                  target_party = party_manager.get_party(guild_id, party_id_to_view) # Needs guild_id

                  if not target_party:
                       await send_callback(f"❌ Партия с ID `{party_id_to_view}` не найдена в этой гильдии.")
                       return
             else:
                  ...
                  if player_char_id is None:
                       await send_callback("❌ У вас нет персонажа или вы не состоите в партии.")
                       return
                  target_party = await party_manager.get_party_by_member_id(player_char_id, guild_id) # Needs guild_id
                  if not target_party:
                       await send_callback("❌ Вы не состоите в партии. Укажите ID партии для просмотра (`/party info <ID партии>`).")
                       return
             ...
             # party_view_service = context.get('party_view_service') # You might need a separate service for this
             # if not party_view_service or not hasattr(party_view_service, 'get_party_info_embed'):
             #      await send_callback("❌ Система просмотра партий временно недоступна.")
             #      return
             ...
             try:
                 ...
                 if member_ids:
                      ...
                      for member_id in member_ids:
                           name = member_id # Default to ID
                           if char_mgr:
                                char = char_mgr.get_character(guild_id, member_id) # Needs guild_id
                                if char: name = getattr(char, 'name', name)
                           if name == member_id and npc_mgr: # Only check NPC if not found as Character
                                npc = npc_mgr.get_npc(guild_id, member_id) # Needs guild_id
                                if npc: name = getattr(npc, 'name', name)
                           member_names.append(f"`{member_id[:6]}` ({name})") # Display truncated ID + name

                 info_message = f"Информация о партии **{party_name}** (ID: `{party_id}`).\n"
                 info_message += f"Лидер: `{leader_id[:6]}`\n" # Display truncated leader ID
                 info_message += f"Участники ({len(member_ids)}): " + (", ".join(member_names) if member_names else "Нет.")

                 # This is just a plain message, replace with embed later
                 await send_callback(info_message)
                 print(f"CommandRouter: Sent party info for party {party_id} in guild {guild_id}.")

                 # If using a PartyViewService:
                 # party_embed = await party_view_service.get_party_info_embed(target_party, context=context)
                 # if party_embed: await send_callback(embed=party_embed)
                 # else: await send_callback(f"❌ Не удалось сгенерировать информацию для партии **{party_name}**.")


             except Exception as e:
                  print(f"CommandRouter Error generating party info for party {getattr(target_party, 'id', 'N/A')} in guild {guild_id}: {e}")
                  import traceback
                  traceback.print_exc()
    ...
    #      """Управляет миром (время, статус и т.д.)."""
    #      send_callback = context['send_to_command_channel']
    #      guild_id = context.get('guild_id')
    ...
    #      if subcommand == "time":
    #           # Handle /world time
    #           current_time = await time_mgr.get_game_time(guild_id) # Needs guild_id
    ...
    #           char = char_mgr.get_character_by_discord_id(guild_id, int(author_id)) # Assuming author_id is always valid here
    #           current_location_id = getattr(char, 'location_id', None) if char else None
    #           if not current_location_id: return await send_callback("❌ Не удалось определить ваше текущее местоположение.")
    #           location_instance = loc_mgr.get_location_instance(guild_id, current_location_id) # Needs guild_id
    ...
    #           # if not loc_view_service: return await send_callback("❌ Система просмотра локаций недоступна.")
    #           # location_embed = await loc_view_service.get_location_info_embed(location_instance, context)
    #           # await send_callback(embed=location_embed)
    #           await send_callback(f"Вы находитесь в локации {getattr(location_instance, 'name', 'Без названия')} (ID: {current_location_id}) в гильдии {guild_id}.")
    #      else:
    ...
    #     # Similar logic to world look subcommand, but maybe allows looking at specific objects (NPCs, items?)
    #     pass

    # @command("interact")
    # async def handle_interact(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
    #     """Взаимодействует с объектом в текущей локации."""
    #     # Needs LocationManager (to find target), potentially DialogueManager, RuleEngine, CharacterActionProcessor
    #     pass

    # @command("attack")
    # async def handle_attack(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
    #     """Атакует цель."""
    #     # Needs CombatManager, CharacterActionProcessor, LocationManager, RuleEngine
    #     pass
    ...
    #     """Инициирует или совершает торговую операцию."""
    #     # Needs EconomyManager, ItemManager, CharacterManager/NpcManager
    #     pass


# Helper function example (can be defined in this file or a utility module)
# def is_uuid_format(s: str) -> bool:
#      """Проверяет, выглядит ли строка как UUID."""
#      # Simple regex check or try/except uuid.UUID
#      try:
#          uuid.UUID(s, version=4) # Check for valid UUID v4
#          return True
#      except (ValueError, AttributeError): # Catch if s is not a string or not a valid UUID
#          return False

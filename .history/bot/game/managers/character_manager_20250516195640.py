...
class CharacterManager:
        ...
        **kwargs: Any # Словарь с дополнительными аргументами (например, guild_id)
        ...
        if existing_char: # Если найден персонаж с этим discord_id
             ...
             if existing_char_guild_id is not None and str(existing_char_guild_id) == str(guild_id_for_create):
                  print(f"CharacterManager: Character already exists for discord ID {discord_id} in guild {guild_id_for_create}. Creation failed.")
    ...
    async def load_state(self, guild_id: str, **kwargs: Any) -> None: # load_state загружает персонажи для определенного guild_id
        ...
        try:
            ...
            # Возможно, нужно перебрасывать исклюжение, чтобы GameManager знал, что загрузка не удалась?
    ...
    async def handle_character_death(self, character_id: str, **kwargs: Any): # Обрабатывает смерть персонажа.
         ...
         send_callback_factory = kwargs.get('send_callback_factory') # Type: Optional[Callable[[int], Callable[[str, Optional[Dict[str, Any]]], Awaitable[Any]]]]
         ...
         # Если send_callback_factory и guild_id доступны, пытаемся уведомить
         if send_callback_factory and guild_id is not None: # Нужны фабрика и guild_id
             ...
             if death_channel_id is None and self._settings is not None:
                 # Пример: ID канала смерти в настройках (по guild_id?)
                 guild_settings = self._settings.get(str(guild_id), self._settings) # Попытка получить per-guild настройки, иначе общие
                 death_channel_id_setting = guild_settings.get('death_channel_id') # Поиск в настройках
                 if death_channel_id_setting is not None:
                      try: death_channel_id = int(death_channel_id_setting) # Попытка преобразовать в int
                      except (ValueError, TypeError): print(f"CharacterManager: Warning: Invalid 'death_channel_id' in settings for guild {guild_id}: {death_channel_id_setting}. Expected integer.");
             ...
             # if death_channel_id is None and self._location_manager and hasattr(self._location_manager, 'get_location_channel') and current_location_id is not None:
             #      # LocationManager.get_location_channel(guild_id, instance_id)
             #      death_channel_id = self._location_manager.get_location_channel(str(guild_id), str(current_location_id)) # Проверить типы аргументов


             if death_channel_id is not None:
                  # Получаем колбэк для канала смерти
                  # send_callback_factory ожидает int channel_id
                  send_callback = send_callback_factory(int(death_channel_id)) # Type: Callable[[str, Optional[Dict[str, Any]]], Awaitable[Any]]
                  # Сообщение о смерти
                  death_message_content: str = f"☠️ Персонаж **{getattr(char, 'name', 'Unknown')}** погиб! ☠️" # Используем getattr для имени

                  try: await send_callback(death_message_content, None)
                  except Exception as e: print(f"CharacterManager: Error sending death message for {character_id} to channel {death_channel_id}: {e}"); import traceback; print(traceback.format_exc());
         ...
         cleanup_context: Dict[str, Any] = {
             'guild_id': guild_id, # Передаем guild_id (полученный из char.guild_id или kwargs)
         ...
         if self._status_manager and hasattr(self._status_manager, 'clean_up_for_entity'): # StatusManager может иметь универсальный clean_up_for_entity
              try: await self._status_manager.clean_up_for_entity(character_id, entity_type="Character", context=cleanup_context) # Передаем context
         ...
         if self._party_manager and hasattr(self._party_manager, 'remove_member'): # Предполагаем метод remove_member(party_id, character_id, context) или clean_up_for_character
              try:
                   ...
                   else: # Party_id is None
                        pass # Персонаж не в группе
         ...
         elif self._party_manager and hasattr(self._party_manager, 'clean_up_for_character'): # Fallback на старое имя
         ...
         if self._dialogue_manager and hasattr(self._dialogue_manager, 'clean_up_for_entity'): # Предполагаем универсальный метод
             try: await self._dialogue_manager.clean_up_for_entity(character_id, entity_type="Character", context=cleanup_context) # Передаем context
         ...
         char_location_id = getattr(char, 'location_id', None)
         if self._item_manager and hasattr(self._item_manager, 'drop_all_inventory') and char_location_id is not None:
              try: await self._item_manager.drop_all_inventory(character_id, entity_type="Character", location_id=char_location_id, context=cleanup_context)
         ...
         elif self._item_manager and hasattr(self._item_manager, 'clean_up_for_character'): # Fallback
              try: await self._item_manager.clean_up_for_character(character_id, context=cleanup_context)
              except Exception as e: print(f"CharacterManager: Error during inventory cleanup for {character_id}: {e}"); import traceback; print(traceback.format_exc());

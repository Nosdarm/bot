...
import json
import uuid # Импорт uuid, т.к. используется для генерации Character ID
import traceback
import asyncio
# Импорт базовых типов
from typing import Optional, Dict, Any, List, Set, TYPE_CHECKING # Import Set, TYPE_CHECKING


# --- Imports ---
# Прямой импорт модели Character, так как она нужна для runtime (например, для Character.from_dict, isinstance)
...
if TYPE_CHECKING:
    ...
    # Это нужно, чтобы Pylance корректно разрешал строковый литерал "Character" в аннотациях (напр., Dict[str, "Character"]).
    from bot.game.models.character import Character
...
class CharacterManager:
    ...
    required_args_for_load: List[str] = ["guild_id"] # load_state фильтрует по guild_id
    required_args_for_save: List[str] = ["guild_id"] # save_state фильтрует по guild_id
    required_args_for_rebuild: List[str] = ["guild_id"] # rebuild_runtime_caches фильтрует по guild_id
    ...
    def get_character_by_name(self, name: str) -> Optional["Character"]:
         ...
         for char in self._characters.values():
             ...
             if getattr(char, 'name', None) == name: # Use getattr safely
                 return char
         return None
    ...
    def get_characters_in_location(self, guild_id: str, location_id: str, **kwargs: Any) -> List["Character"]:
        ...
        for char in self._characters.values():
            ...
            if hasattr(char, 'guild_id') and str(getattr(char, 'guild_id', None)) == guild_id_str and \
               hasattr(char, 'location_id') and str(getattr(char, 'location_id', None)) == location_id_str:
    ...
    def is_busy(self, character_id: str) -> bool:
        ...
        if getattr(char, 'party_id', None) is not None and self._party_manager and hasattr(self._party_manager, 'is_party_busy'):
            ...
            # Если PartyManager.is_party_busy ожидает guild_id, нужно получить его из Character объекта:
            guild_id_char = getattr(char, 'guild_id', None)
            # Если guild_id_char доступен и не None, можно его передать, если PartyManager ожидает.
            # if guild_id_char is not None:
            #      # Передаем guild_id, если PartyManager.is_party_busy ожидает его
            ...
            try:
                 return self._party_manager.is_party_busy(getattr(char, 'party_id')) # Call with party_id
            except Exception as e:
                 # Log error if party_id exists but call failed
                 print(f"CharacterManager: Error checking party busy status for character {character_id} (party {getattr(char, 'party_id', 'N/A')}): {e}")
                 import traceback
        ...
        **kwargs: Any # Словарь с дополнительными аргументами (например, guild_id)
        ...
        if existing_char: # Если найден персонаж с этим discord_id
             ...
             if existing_char_guild_id is not None and str(existing_char_guild_id) == str(guild_id_for_create):
                  ...
                  print(f"CharacterManager: Character with discord ID {discord_id} already exists (ID: {getattr(existing_char, 'id', 'N/A')}) in guild {getattr(existing_char, 'guild_id', 'N/A')}. Creation failed.")
                  # Возможно, нужно отправить уведомление GM или игроку?
                  raise ValueError(f"User already has a character (ID: {getattr(existing_char, 'id', 'N/A')}) in guild {getattr(existing_char, 'guild_id', 'N/A')}).")
        ...
        if self._rule_engine and hasattr(self._rule_engine, 'generate_initial_character_stats'):
            try:
                ...
                # Example: await self._rule_engine.generate_initial_character_stats(context=kwargs)
        ...
        try:
            ...
            print(f"CharacterManager: Character '{name}' (ID: {char.id}, Guild: {getattr(char, 'guild_id', 'N/A')}) created and cached.") # Логируем guild_id
    ...
    async def load_state(self, guild_id: str, **kwargs: Any) -> None: # load_state загружает персонажи для определенного guild_id
        ...
        if self._db_adapter is None:
            ...
            # Это должно быть реализовано внутри load_state, если нет db_adapter.
            # Пока просто возвращаемся, если нет DB.
            return
        ...
        # Это приведет к тому, что при загрузке нескольких гильдий в кеше останутся только персонажи последней загруженной гильгии.
        self._characters.clear() # Очистка глобального кеша
        self._discord_to_char_map.clear() # Очистка глобальной мапы
        self._entities_with_active_action.clear() # Очистка глобального Set
        if hasattr(self, '_dirty_characters') and isinstance(self._dirty_characters, Set): self._dirty_characters.clear() # Очищаем, так как все из DB считается чистым
        if hasattr(self, '_deleted_characters_ids') and isinstance(self._deleted_characters_ids, Set): self._deleted_characters_ids.clear() # Очищаем
        ...
        try:
            ...
            # Лучше просто логировать и не загружать персонажей для этой гильгии.
    ...
    def rebuild_runtime_caches(self, guild_id: str, **kwargs: Any) -> None: # Перестройка кешей, специфичных для выполнения, после загрузки состояния для гильдии.
        ...
        # Этот метод в текущей структуре кешей не делает ничего, т.с. кеши (_characters, _discord_to_char_map)
    ...
    async def remove_character(self, character_id: str, guild_id: str, **kwargs: Any) -> Optional[str]:
        ...
        print(f"CharacterManager: Removing character {character_id} ({getattr(char, 'name', 'N/A')}) from guild {guild_id}...") # Use getattr safely for name
        ...
        try:
            ...
            # Предполагаем, что они принимают context и character_id.
            ...
            # StatusManager может иметь clean_up_for_entity
            if self._status_manager and hasattr(self._status_manager, 'clean_up_for_entity'):
                 try: await self._status_manager.clean_up_for_entity(character_id, entity_type="Character", context=cleanup_context) # Передаем entity_type
            ...
            elif self._status_manager and hasattr(self._status_manager, 'clean_up_for_character'): # Fallback на старое имя
                 try: await self._status_manager.clean_up_for_character(character_id, context=cleanup_context)
                 except Exception as e: print(f"CharacterManager: Error during status cleanup for {character_id}: {e}"); import traceback; print(traceback.format_exc());

            # PartyManager может иметь clean_up_for_character
            if self._party_manager and hasattr(self._party_manager, 'clean_up_for_character'):
                 try: await self._party_manager.clean_up_for_character(character_id, context=cleanup_context)
            ...
            # CombatManager может иметь remove_participant_from_combat
            if self._combat_manager and hasattr(self._combat_manager, 'remove_participant_from_combat'): # Предполагаем метод
                 try: await self._combat_manager.remove_participant_from_combat(character_id, entity_type="Character", context=cleanup_context) # Передаем entity_type
            ...
            elif self._combat_manager and hasattr(self._combat_manager, 'clean_up_for_character'): # Fallback на старое имя
                 try: await self._combat_manager.clean_up_for_character(character_id, context=cleanup_context)
                 except Exception as e: print(f"CharacterManager: Error during combat cleanup for {character_id}: {e}"); import traceback; print(traceback.format_exc());

            # DialogueManager может иметь clean_up_for_entity
            if self._dialogue_manager and hasattr(self._dialogue_manager, 'clean_up_for_entity'): # Предполагаем универсальный метод
                 try: await self._dialogue_manager.clean_up_for_entity(character_id, entity_type="Character", context=cleanup_context) # Передаем entity_type
            ...
            elif self._dialogue_manager and hasattr(self._dialogue_manager, 'clean_up_for_character'): # Fallback
                 try: await self._dialogue_manager.clean_up_for_character(character_id, context=cleanup_context)
                 except Exception as e: print(f"CharacterManager: Error during dialogue cleanup for {character_id}: {e}"); import traceback; print(traceback.format_exc());
            ...
            print(f"CharacterManager: Cleanup initiated for character {character_id} in guild {guild_id}.")
             ...
             # Пока оставляю логирование без перебрасывания.
        ...
        print(f"CharacterManager: Character {character_id} ({getattr(char, 'name', 'N/A')}) removal process initiated for guild {guild_id}. Will be deleted from DB on next save.")
    ...
    async def set_party_id(self, character_id: str, party_id: Optional[str], **kwargs: Any) -> bool:
        ...
        if not char:
            ...
            return False
        if getattr(char, 'party_id', None) == party_id: # Используем getattr для безопасной проверки
        ...
        if hasattr(char, 'party_id'):
            ...
            print(f"CharacterManager: Warning: Character model for {character_id} is missing 'party_id' attribute. Cannot set party ID.")
    ...
    async def update_character_location(self, character_id: str, location_id: Optional[str], **kwargs: Any) -> bool:
        ...
        if hasattr(char, 'location_id'):
            char.location_id = location_id
             ...
             print(f"CharacterManager: Warning: Character model for {character_id} is missing 'location_id' attribute. Cannot update location.")
    ...
    async def update_health(self, character_id: str, amount: float, **kwargs: Any) -> bool:
         ...
         if not hasattr(char, 'health') or not isinstance(getattr(char, 'health', None), (int, float)): # Use getattr safely
              ...
              return False
         if not hasattr(char, 'max_health') or not isinstance(getattr(char, 'max_health', None), (int, float)): # Use getattr safely
              ...
              return False
         if not hasattr(char, 'is_alive') or not isinstance(getattr(char, 'is_alive', None), bool): # Use getattr safely
    ...
    async def handle_character_death(self, character_id: str, **kwargs: Any): # Обрабатывает смерть персонажа.
         ...
         if hasattr(char, 'is_alive'):
             ...
             # Решаем, что делать, если не можем пометить как мертвого. Log and proceed? Raise? Log and return?
             # Давайте просто логируем и возвращаемся.
             return # Не удалось пометить как мертвого


         self.mark_character_dirty(character_id) # Помечаем измененным

         print(f"Character {character_id} ({getattr(char, 'name', 'N/A')}) has died in guild {getattr(char, 'guild_id', 'N/A')}.") # Log name and guild_id safely


        # ... (остальная часть handle_character_death с логикой очистки и уведомления) ...

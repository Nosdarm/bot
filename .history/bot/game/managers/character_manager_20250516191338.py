...
# --- Imports ---
# Прямой импорт модели Character, так как она нужна для runtime (Character.from_dict)
from bot.game.models.character import Character

# Прямой импорт SqliteAdapter, если он используется в isinstance или не только в аннотациях
# Если он используется только для аннотаций в __init__, можно перенести его в TYPE_CHECKING.
# Судя по предыдущему коду, вы используете self._db_adapter (инстанс), а не класс SqliteAdapter напрямую.
# Переместим его в TYPE_CHECKING.
# from bot.database.sqlite_adapter import SqliteAdapter # Удаляем прямой импорт


# TYPE_CHECKING импорты для избежания циклических зависимостей и для улучшения статического анализа
# Используем строковые литералы для всех типов, которые могут вызывать проблемы или импортируются условно.
if TYPE_CHECKING:
    ...
    # Это нужно, чтобы Pylance разрешал строковые литералы в аннотациях.
    from bot.game.models.character import Character # <-- Add Character here
...
class CharacterManager:
    ...
    def __init__(
        ...
        # Internal caches
        # Используем строковый литерал в аннотации кеша
    ...
    def get_character(self, character_id: str) -> Optional["Character"]: # Ln 70
        """Получить персонажа по его внутреннему ID (UUID)."""
        return self._characters.get(character_id)

    # Используем строковый литерал в аннотации возвращаемого типа
    def get_character_by_discord_id(self, discord_user_id: int) -> Optional["Character"]: # Ln 79
    ...
    # Используем строковый литерал в аннотации возвращаемого типа
    def get_character_by_name(self, name: str) -> Optional["Character"]: # Ln 83
    ...
    # Используем строковый литерал в аннотации возвращаемого типа
    def get_all_characters(self) -> List["Character"]: # Ln 89
        """Получить список всех загруженных персонажей."""
        return list(self._characters.values())

    def get_entities_with_active_action(self) -> Set[str]: # Ln 97 - в коде нет Character
    ...
    def is_busy(self, character_id: str) -> bool:
        ...
        char = self.get_character(character_id) # Ln 102 (approx)
        if not char:
            return False
    ...
    async def create_character( # Переименован из create_character_for_user
        ...
        **kwargs: Any
    ) -> Optional["Character"]: # Используем строковый литерал в аннотации возвращаемого типа - Ln 130
        """
        ...
        try:
            ...
            char = Character.from_dict(data) # Ln 236
            ...
            print(f"CharacterManager: Error creating character '{name}' for discord ID {discord_id}: {e}")
            traceback.print_exc()
    ...
    async def save_state(self, guild_id: str, **kwargs: Any) -> None: # Переименован из save_all_characters и добавил guild_id и kwargs
        ...
        deleted_char_ids_for_guild = {cid for cid in self._deleted_characters_ids if cid not in self._characters} # Проверяем, что его нет в кеше (уже удален из _characters)
    ...
    async def load_state(self, guild_id: str, **kwargs: Any) -> None: # Переименован из load_all_characters и добавил guild_id и kwargs
        ...
        try:
            ...
            print(f"CharacterManager: ❌ CRITICAL ERROR executing DB fetchall для guild {guild_id}: {e}") # Уточнено логирование
            traceback.print_exc()
        ...
        for row in rows:
            ...
            try:
                ...
                data['guild_id'] = loaded_guild_id # Сохраняем string версию


                # Создаем объект модели Character
                # Передаем полный словарь data, который теперь содержит все преобразованные значения
                char = Character.from_dict(data) # Ln 461 (approx)

                # Добавляем в кеш по ID
                self._characters[char.id] = char

                # Добавляем в кеш по Discord ID, только если discord_user_id валидный int
                if discord_user_id_int is not None:
                     self._discord_to_char_map[discord_user_id_int] = char.id
    ...
    def rebuild_runtime_caches(self, guild_id: str, **kwargs: Any) -> None: # Добавлена аннотация **kwargs
        ...
        # e.g., party_mgr = kwargs.get('party_manager') # type: Optional["PartyManager"] # Используем строковый литерал и в комментариях
    ...
    async def handle_character_death(self, character_id: str, **kwargs: Any):
         ...
         # combat_mgr = kwargs.get('combat_manager') or self._combat_manager # type: Optional["CombatManager"] # Используем строковый литерал
         # if combat_mgr and hasattr(combat_mgr, 'remove_from_combat'):
         #      await combat_mgr.remove_from_combat(character_id, context=kwargs)

         # party_mgr = kwargs.get('party_manager') or self._party_manager # type: Optional["PartyManager"] # Используем строковый литерал

...
from discord import Message # Used in route method signature
...
if TYPE_CHECKING:
    ...
    # ИСПРАВЛЕНИЕ: Добавляем Character здесь для разрешения строкового литерала
...
class CommandRouter:
    ...
    async def route(self, message: Message) -> None:
        ...
        try:
            ...
            if not command_line: # Message was just the prefix
                 ...
                 # await self._send_callback_factory(message.channel.id)(f"Привет! Я бот {self._discord_client.user.name}. Используй `{self._command_prefix} help` для списка команд.", None)
            ...
            try:
                 ...
                 await send_callback(f"❌ Ошибка при разборе команды: {e}", None)
        ...
        if not handler:
            ...
            try:
                 send_callback = self._send_callback_factory(message.channel.id)
                 await send_callback(f"❓ Неизвестная команда: `{self._command_prefix}{command_keyword}`. Используйте `{self._command_prefix}help` для просмотра доступных команд.", None)
            except Exception as cb_e:
        ...
        try:
            ...
            try:
                 send_callback = self._send_callback_factory(message.channel.id)
                 await send_callback(f"❌ Произошла ошибка при выполнении команды `{self._command_prefix}{command_keyword}`: {e}", None)
            except Exception as cb_e:
    ...
    async def handle_help(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
        ...
        if not args:
            ...
            await send_callback(help_message, None)
        else:
            ...
            if handler:
                ...
                await send_callback(f"❓ Команда `{target_command}` не найдена.", None)
    ...
    async def handle_create_character(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
        ...
        if guild_id is None:
            await send_callback("❌ Создание персонажа возможно только на сервере.", None)
            return

        if not args:
            await send_callback(f"Использование: `{self._command_prefix}create_character <имя_персонажа>`", None)
            return
        ...
        if not char_manager:
             await send_callback("❌ Система персонажей временно недоступна.", None)
        ...
        try:
            ...
            try:
                ...
                await send_callback("❌ Не удалось определить ваш ID пользователя Discord.", None)
                print(f"CommandRouter Error: Invalid author_id format: {author_id}")
                return

            if author_id_int is None:
                 await send_callback("❌ Не удалось получить ваш ID пользователя Discord.", None)
            ...
            if new_character:
                ...
                await send_callback(f"✨ Персонаж **{char_name}** успешно создан! (ID: `{char_id}`)", None)
                ...
                await send_callback(f"❌ Не удалось создать персонажа **{character_name}**. Возможно, имя занято или у вас уже есть персонаж в этой гильдии.", None)
             ...
             await send_callback(f"❌ Ошибка создания персонажа: {ve}", None)
            ...
            import traceback
            traceback.print_exc()
            await send_callback(f"❌ Произошла ошибка при создании персонажа: {e}", None)
    ...
    async def handle_stats(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
        ...
        if guild_id is None:
            await send_callback("❌ Эту команду можно использовать только на сервере.", None)
            return

        char_id_to_view: Optional[str] = None
        # ИСПРАВЛЕНИЕ: Type hint for target_char resolved by adding import in TYPE_CHECKING
        ...
        if not char_manager or not char_view_service:
             await send_callback("❌ Система персонажей или просмотра временно недоступна.", None)
        ...
        if args:
            ...
            if not target_char:
                 await send_callback(f"❌ Персонаж с ID `{char_id_to_view}` не найден в этой гильдии.", None)
            ...
            if player_char:
                ...
                await send_callback("❌ У вас еще нет персонажа. Создайте его командой `{self._command_prefix}create_character <имя>`", None)
        ...
        if target_char is None: # This check is technically redundant due to returns above, but safe.
             await send_callback("❌ Не удалось определить, чьи статы показать.", None)
             return
        ...
        try:
            ...
            if stats_embed:
                 ...
                 await send_callback(f"❌ Не удалось сгенерировать статистику для персонажа **{getattr(target_char, 'name', 'N/A')}**.", None) # Get name safely
            ...
            import traceback
            traceback.print_exc()
            await send_callback(f"❌ Произошла ошибка при получении статистики: {e}", None)
    ...
    async def handle_inventory(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
        ...
        # At this point, target_char should be a Character object if found, otherwise we returned early.
        if target_char is None: # This check is technically redundant due to returns above, but safe.
             await send_callback("❌ Не удалось определить, чей инвентарь показать.", None)
             return
        ...
        try:
            ...
            if inventory_embed:
                 ...
                 await send_callback(f"❌ Не удалось сгенерировать инвентарь для персонажа **{getattr(target_char, 'name', 'N/A')}**.", None)
            ...
            import traceback
            traceback.print_exc()
            await send_callback(f"❌ Произошла ошибка при получении инвентаря: {e}", None)
    ...
    async def handle_move(self, message: Message, args: List[str], context: Dict[str, Any]) -> None:
        ...
        if not args:
            await send_callback(f"Использование: `{self._command_prefix}move <ID локации>`", None)
            return
        ...
        if not char_manager or not char_action_processor:
             await send_callback("❌ Система перемещения или персонажей временно недоступна.", None)
        ...
        player_char = char_manager.get_character_by_discord_id(guild_id, author_id_int) # Need guild_id and convert author_id to int
        if not player_char:
            await send_callback("❌ У вас еще нет персонажа.", None)
            return

        char_id = getattr(player_char, 'id', None) # Get character ID safely
        if char_id is None:
             await send_callback("❌ Не удалось определить ID вашего персонажа.", None)
             return
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            await send_callback(f"❌ Произошла ошибка при попытке перемещения: {e}", None)
    ...
    #          await send_callback("❌ Система партий временно недоступна.", None)
    ...
    #          await send_callback(f"Использование: `{self._command_prefix}join_party <ID партии>`", None)
    #          return
    #
    ...
    #     if not target_party:
    #          await send_callback(f"❌ Партия с ID `{target_party_id}` не найдена в этой гильдии.", None)
    #          return
    #
    #     try:
    ...
    #         import traceback
    #         traceback.print_exc()
    #         await send_callback(f"❌ Произошла ошибка при попытке присоединиться к партии: {e}", None)

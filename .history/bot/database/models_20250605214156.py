...
# Removed potential duplicate Base = declarative_base() from end of file if present
...
class ItemTemplate(Base):
    ...
    name = Column(String, nullable=False)
    description = Column(Text, nullable=True)
    properties = Column(JSON, nullable=True) # For various item attributes like weight, damage, effects
    # Consider adding other relevant fields if known, e.g.:
    # slot = Column(String, nullable=True) 
    # value = Column(Integer, nullable=True)
    # type = Column(String, nullable=True) # e.g., 'weapon', 'potion', 'armor'

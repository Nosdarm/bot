...
class SqliteAdapter:
    ...
    async def _migrate_v8_to_v9(self, cursor: Cursor) -> None:
        ...
        hp_exists = 'hp' in column_names
        health_exists = 'health' in column_names

        if health_exists and hp_exists:
            print("SqliteAdapter: Both 'health' and 'hp' exist. Consolidating into 'hp' and dropping 'health'.")
            # Prioritize 'health' data if 'hp' is default or different.
            # This assumes 'hp' might have been added with a default value in v8.
            ...
            # Define the schema for players_new EXPLICITLY, excluding 'health'
            # This is based on the expected schema after all prior migrations.
            # Original fields from 'characters' (v0) minus 'health', plus additions from v3_to_v4, plus 'hp' from v7_to_v8
            new_table_columns = [
                "id TEXT PRIMARY KEY",
                ...
                "stats TEXT DEFAULT '{}'",
                "inventory TEXT DEFAULT '[]'", # This was an old way, actual inventory is in 'inventory' table
                "current_action TEXT NULL",
                "action_queue TEXT DEFAULT '[]'",
                "party_id TEXT NULL",
                "state_variables TEXT DEFAULT '{}'",
                # 'health REAL DEFAULT 100.0', -- This is being dropped
                "max_health REAL DEFAULT 100.0", # This should exist
                ...
                # Columns added in v3_to_v4 for 'players'
                "race TEXT",
                "mp INTEGER DEFAULT 0",
                "attack INTEGER DEFAULT 0",
                "defense INTEGER DEFAULT 0",
                # 'hp' column confirmed/added in v7_to_v8
                "hp REAL DEFAULT 100.0"
            ]
            
            # Define UNIQUE constraints separately as they are table-level
            unique_constraints = "UNIQUE(discord_user_id, guild_id), UNIQUE(name, guild_id)"
            
            create_new_sql = f"CREATE TABLE players_new ({', '.join(new_table_columns)}, {unique_constraints});"

            # Columns for the SELECT and INSERT statement (all columns in players_new)
            # Ensure these match the order and names in new_table_columns (excluding types and constraints)
            select_insert_columns_list = [
                "id", "discord_user_id", "name", "guild_id", "location_id", "stats", "inventory",
                "current_action", "action_queue", "party_id", "state_variables",
                "max_health", "is_alive", "status_effects", "level", "experience", "active_quests",
                "created_at", "last_played_at",
                "race", "mp", "attack", "defense", "hp"
            ]
            select_insert_columns_str = ", ".join(select_insert_columns_list)

            await cursor.execute(create_new_sql)
            print(f"SqliteAdapter: Created players_new table with schema: {create_new_sql}")
            
            # Data is already consolidated into 'hp'. Now select all columns *except* the old 'health'.
            await cursor.execute(f"INSERT INTO players_new ({select_insert_columns_str}) SELECT {select_insert_columns_str} FROM players;")
            print(f"SqliteAdapter: Copied data to players_new using columns: {select_insert_columns_str}")
        ...
        elif hp_exists and not health_exists:
            print("SqliteAdapter: 'hp' column exists and 'health' column does not. Schema is already correct.")
        
        elif not hp_exists and health_exists:
            ...
            print("SqliteAdapter: CRITICAL - Neither 'hp' nor 'health' found. Migration v7_to_v8 should have added 'hp'.")
            # Attempt to add 'hp' as a last resort, though this indicates a deeper issue if v7_to_v8 didn't run or failed.
            if "hp" not in column_names:
                try:
                    await cursor.execute("ALTER TABLE players ADD COLUMN hp REAL DEFAULT 100.0;")
                    print("SqliteAdapter: Added 'hp' column as a fallback because it was completely missing.")
                except Exception as e_add_hp_fallback:
                     print(f"SqliteAdapter: Fallback attempt to add 'hp' column failed: {e_add_hp_fallback}")
                     traceback.print_exc() 
                     raise # This is critical if 'hp' cannot be ensured

...
class SqliteAdapter:
    ...
    async def _migrate_v0_to_v1(self, cursor: Cursor) -> None:
        ...
        await cursor.execute(sql_characters) # Эта строка вызывает ошибку


        # ... (остальные CREATE TABLE для других таблиц ниже) ...

        sql_events = '''
            CREATE TABLE IF NOT EXISTS events (
                id TEXT PRIMARY KEY,
        # ... и так далее для всех остальных таблиц ...
        '''
        print(f"DEBUG: Executing CREATE TABLE events SQL:\n---\n{sql_events}\n---") # Добавьте аналогичную печать для каждой таблицы

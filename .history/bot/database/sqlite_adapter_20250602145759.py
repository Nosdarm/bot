...
class SqliteAdapter:
    ...
    LATEST_SCHEMA_VERSION = 12 # Added i18n columns to locations, item_templates, npcs
    ...
    async def _migrate_v3_to_v4(self, cursor: Cursor) -> None:
        ...
        try:
            ...
            # If players table already exists, check for selected_language column
            ...
            if 'selected_language' not in column_names:
                try:
                    await cursor.execute("ALTER TABLE players ADD COLUMN selected_language TEXT DEFAULT NULL;")
                    print("SqliteAdapter: Added column 'selected_language' to existing 'players' table in v3_to_v4.")
                except aiosqlite.OperationalError as e_add_lang:
                    if "duplicate column name" not in str(e_add_lang).lower():
                        raise # Re-raise other operational errors
                    else:
                        print("SqliteAdapter: Column 'selected_language' already exists in 'players' table (checked in v3_to_v4).")
            else:
                print("SqliteAdapter: Column 'selected_language' already present in 'players' table (checked in v3_to_v4).")
        ...
        player_columns_to_add = [
            ...
            ("selected_language", "TEXT DEFAULT NULL") # Added for v11, ensure it's here if table is new
        ]
    ...
    async def _migrate_v8_to_v9(self, cursor: Cursor) -> None:
        ...
        if 'health' in column_names and 'hp' in column_names:
            ...
            try:
                ...
                # Also include selected_language
                ...
                CREATE TABLE players_new (
                    ...
                    selected_language TEXT DEFAULT NULL, -- Added for v11
                    UNIQUE(discord_user_id, guild_id),
                    UNIQUE(name, guild_id)
                );
                """
                await cursor.execute(create_players_new_sql)
                print("SqliteAdapter: Created 'players_new' table with standardized schema (no 'health' column, includes 'selected_language').")

                # Explicitly list columns for insertion, ensuring 'hp' gets its (potentially updated) value
                # and 'health' is excluded. Include selected_language.
                # If selected_language doesn't exist in the old 'players' table, it will be NULL, which is fine.
                # We need to check if selected_language exists in the source 'players' table before selecting it.
                await cursor.execute("PRAGMA table_info(players);")
                source_columns_info = await cursor.fetchall()
                source_column_names = [row['name'] for row in source_columns_info if row and 'name' in row.keys()]

                cols_to_select_list = [
                    "id", "discord_user_id", "name", "guild_id", "location_id", "stats",
                    "inventory", "current_action", "action_queue", "party_id", "state_variables",
                    "max_health", "is_alive", "status_effects", "level", "experience",
                    "active_quests", "created_at", "last_played_at", "race", "mp",
                    "attack", "defense", "hp"
                ]
                if 'selected_language' in source_column_names:
                    cols_to_select_list.append("selected_language")
                    print("SqliteAdapter: 'selected_language' column found in source players table for v8_to_v9 copy.")
                else:
                    # If it's not in source, it will get its DEFAULT NULL value in players_new
                    print("SqliteAdapter: 'selected_language' column NOT found in source players table for v8_to_v9 copy. Will use DEFAULT in new table.")


                cols_to_insert_into = cols_to_select_list # Columns are the same in new table
                
                # Construct the SELECT part carefully. If selected_language is missing from source,
                # we must not try to select it. It will get its default value in the new table.
                select_expressions = cols_to_select_list[:] # Make a copy

                # If selected_language was NOT in source_column_names, we don't need to do anything special for the
                # INSERT INTO players_new (...) SELECT ... FROM players;
                # because the players_new.selected_language column has a DEFAULT NULL.
                # SQLite will handle this. The key is that 'selected_language' MUST be in the
                # target column list for players_new if it's in the CREATE TABLE statement.

                cols_for_insert_clause = ", ".join(cols_to_insert_into)
                cols_for_select_clause = ", ".join(select_expressions)

                insert_sql = f"INSERT INTO players_new ({cols_for_insert_clause}) SELECT {cols_for_select_clause} FROM players;"
    ...
    async def _migrate_v10_to_v11(self, cursor: Cursor) -> None:
        """Миграция с Версии 10 на Версию 11 (добавление selected_language в players)."""
        print("SqliteAdapter: Running v10 to v11 migration (add selected_language to players)...")
        ...
        if 'selected_language' not in column_names:
            try:
                await cursor.execute("ALTER TABLE players ADD COLUMN selected_language TEXT DEFAULT NULL;")
                print("SqliteAdapter: Added column 'selected_language' (TEXT DEFAULT NULL) to 'players' table.")
            except aiosqlite.OperationalError as e:
                # This specific check for "duplicate column name" might be redundant if the outer check works,
                # but it's good for robustness.
                if "duplicate column name" in str(e).lower():
                    print("SqliteAdapter: Column 'selected_language' already exists in 'players' table (v10_to_v11).")
                else:
                    print(f"SqliteAdapter: Error adding 'selected_language' to 'players' table: {e}")
                    traceback.print_exc()
                    raise
        else:
            print("SqliteAdapter: Column 'selected_language' already exists in 'players' table (v10_to_v11).")
    ...
    async def _migrate_v11_to_v12(self, cursor: Cursor) -> None:
        """Миграция с Версии 11 на Версию 12 (добавление колонок *_i18n)."""
        print("SqliteAdapter: Running v11 to v12 migration (adding i18n columns)...")

        default_lang = "en"

        tables_to_update = {
            "locations": ["name", "description"],
            "item_templates": ["name", "description"],
            "npcs": ["name", "description"] 
            # "persona" in npcs is not included for now as per plan.
        }

        for table_name, columns in tables_to_update.items():
            for column_name in columns:
                i18n_column_name = f"{column_name}_i18n"
                
                # 1. Add the new i18n column
                try:
                    await cursor.execute(f"ALTER TABLE {table_name} ADD COLUMN {i18n_column_name} TEXT DEFAULT '{{}}';")
                    print(f"SqliteAdapter: Added column '{i18n_column_name}' to '{table_name}' table.")
                except aiosqlite.OperationalError as e:
                    if "duplicate column name" in str(e).lower():
                        print(f"SqliteAdapter: Column '{i18n_column_name}' already exists in '{table_name}' table, skipping ALTER TABLE.")
                    else:
                        raise
                
                # 2. Populate the new i18n column from the old column
                await cursor.execute(f"SELECT id, {column_name} FROM {table_name};")
                ...
                for row in rows_to_migrate:
                    ...
                    if original_value is not None:
                        i18n_value = json.dumps({default_lang: original_value})
                        update_data.append((i18n_value, row_id))
                
                if update_data:
                    await cursor.executemany(f"UPDATE {table_name} SET {i18n_column_name} = ? WHERE id = ?;", update_data)
                    print(f"SqliteAdapter: Migrated data from '{column_name}' to '{i18n_column_name}' for {len(update_data)} rows in '{table_name}'.")
                else:
                    print(f"SqliteAdapter: No data to migrate for '{column_name}' in '{table_name}'.")

...
def run_migrations_offline() -> None:
    ...
    # --- Start Correctly Placed Diagnostic Exception (OFFLINE) ---
    import sys # Ensure sys is imported
    # Ensure target_metadata is accessible
    tables_message_offline = f"DIAGNOSTIC EXCEPTION (OFFLINE): Tables in target_metadata: {target_metadata.tables.keys() if target_metadata else 'target_metadata is None'}"
    raise RuntimeError(tables_message_offline)
    # --- End Correctly Placed Diagnostic Exception (OFFLINE) ---
    context.configure(
        url=url,
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        # --- Start Correctly Placed Diagnostic Exception ---
        import sys # Ensure sys is imported, or use existing import
        # Ensure target_metadata is accessible here (it's global in env.py)
        tables_message_online = f"DIAGNOSTIC EXCEPTION (ONLINE): Tables in target_metadata: {target_metadata.tables.keys() if target_metadata else 'target_metadata is None'}"
        raise RuntimeError(tables_message_online)
        # --- End Correctly Placed Diagnostic Exception ---
        context.configure(

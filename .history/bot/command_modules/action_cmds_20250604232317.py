...
async def cmd_fight(interaction: Interaction, target_npc_name: Optional[str] = None):
    ...
    try:
        if not hasattr(interaction.client, 'game_manager') or not interaction.client.game_manager:
            ...
            return
        game_mngr_instance = interaction.client.game_manager
        if not hasattr(game_mngr_instance, 'db_service') or not game_mngr_instance.db_service:
            await interaction.followup.send("Error: DBService is not available.", ephemeral=True)
            return
        if not hasattr(game_mngr_instance, 'combat_manager') or not game_mngr_instance.combat_manager:
            await interaction.followup.send("Error: CombatManager is not available.", ephemeral=True)
            return
        # Also check for npc_manager and character_manager as they are used for fetching names
        if not hasattr(game_mngr_instance, 'npc_manager') or not game_mngr_instance.npc_manager:
            await interaction.followup.send("Error: NpcManager is not available.", ephemeral=True)
            return
        if not hasattr(game_mngr_instance, 'character_manager') or not game_mngr_instance.character_manager:
...
async def cmd_talk(interaction: Interaction, npc_name: str, message: str):
    ...
    try:
        ...
        if not hasattr(game_mngr_instance, 'db_service') or not game_mngr_instance.db_service:
            await interaction.followup.send("Error: DBService is not available.", ephemeral=True)
            return
        if not hasattr(game_mngr_instance, 'openai_service') or not game_mngr_instance.openai_service:
            await interaction.followup.send("Error: OpenAIService is not available.", ephemeral=True)
            return
        # Also check for npc_manager as it's used for fetching NPC data
        if not hasattr(game_mngr_instance, 'npc_manager') or not game_mngr_instance.npc_manager:
            await interaction.followup.send("Error: NpcManager is not available.", ephemeral=True)
            return
...
@app_commands.command(name="end_turn", description="Завершить свой ход (для соло игроков).")
async def cmd_end_turn(interaction: Interaction):
    ...
    if not hasattr(game_mngr, 'db_service') or not game_mngr.db_service:
    ...
    if not channel_id:
        await interaction.followup.send("Ошибка: Эту команду нельзя использовать в личном сообщении без канала.", ephemeral=True)
        return

    player_char_obj = await game_mngr.get_player_by_discord_id(discord_id=discord_user_id, guild_id=guild_id)

    if not player_char_obj:
        await interaction.followup.send("Сначала вам нужно создать персонажа! Используйте `/start_new_character`.", ephemeral=True)
        return

    # Convert Character object to a dict-like structure if DBService methods expect dicts,
    # or ensure DBService methods can handle Character objects if they expect specific fields.
    # For this example, we'll assume player_char_obj has the necessary attributes like a dict.
    # player_id = player_char_obj.id
    # current_party_id = player_char_obj.current_party_id

    # Direct attribute access assuming player_char_obj is the Character model instance
    player_id = player_char_obj.id
    current_party_id = player_char_obj.current_party_id # Assuming Character model has 'current_party_id'

    if current_party_id:
        await interaction.followup.send("Вы состоите в группе. Пожалуйста, используйте команду `/end_party_turn` для завершения хода группы.", ephemeral=True)
        return

    # Update player status to 'ожидание_обработку'
    update_success = await db_service.update_player_field(
        player_id=player_id,
        field_name='current_game_status',
        value='ожидание_обработку',
        guild_id=guild_id
    )

    if not update_success:
        await interaction.followup.send("Не удалось обновить ваш статус. Попробуйте еще раз.", ephemeral=True)
        return

    # Call GameManager to process the solo player's turn
    # This method needs to be created in GameManager
    await game_mngr.process_solo_player_turn(player_id, guild_id, channel_id)
    
    await interaction.followup.send("Ваши действия приняты и обрабатываются.", ephemeral=True)


@app_commands.command(name="end_party_turn", description="Завершить ход для вашей группы.")
async def cmd_end_party_turn(interaction: Interaction):
    ...
    player_id = player_char_obj.id
    party_id = player_char_obj.current_party_id # Assuming Character model has 'current_party_id'
    location_id = player_char_obj.location_id # Assuming Character model has 'location_id'

    if not party_id:
        await interaction.followup.send("Вы не состоите в группе. Если вы играете соло, используйте `/end_turn`.", ephemeral=True)
        return
    
    if not location_id:
        await interaction.followup.send("Ошибка: Не удалось определить вашу текущую локацию. Пожалуйста, сообщите администратору.", ephemeral=True)
        return
    ...
    # Call GameManager to check if all party members have ended their turn and trigger processing
    # This method needs to be created in GameManager
    await game_mngr.check_and_trigger_party_turn(party_id, location_id, guild_id, channel_id)

    await interaction.followup.send("Вы успешно завершили свой ход в группе. Ожидаем остальных...", ephemeral=True)

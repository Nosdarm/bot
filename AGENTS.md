...
- **Focus:** Re-addressing all 88 errors in `bot/command_modules/inventory_cmds.py`.
- **Strategy:** Overwrote the file with corrected content.
- **Batch 21 Fixes (88 errors in `bot/command_modules/inventory_cmds.py`):**
    - **Language Handling:**
        ...
        - Standardized language determination: character's preference > interaction locale > default bot language.
    - **i18n Calls:**
        - Removed `default_text` parameter from `get_i18n_text` calls.
        - Ensured `guild_id` is passed to `get_i18n_text` where appropriate.
    ...
    - **Manager and Service Handling:**
        - Ensured `game_mngr` and its sub-managers (`character_manager`, `item_manager`, `location_manager`, `rule_engine`) are checked for `None` before use (using `hasattr` and `is not None`).
        - Used `getattr(self.bot, "nlu_data_service", None)` for safer access to `nlu_data_service`.
        - Added explicit `cast()` for managers after `None` checks.
    - **Item and Inventory Logic:**
        - Resolved `Cannot access attribute "get" for class "Item"` by using direct attribute access on item template dictionaries (e.g., `item_template_data.get('name_i18n', {})`).
        - Corrected parameter passing to `item_manager.use_item` (ensured `character_user`, `target_entity_id`, `target_entity_type` are provided).
        - Ensured correct types (string IDs, float quantity) for `item_manager.transfer_item_world_to_character` and `item_manager.unequip_item`.
        - Fixed `Cannot assign to attribute "inventory" for class "Character"`:
            - Ensured `character.inventory` (if read as a JSON string) is parsed to `List[Dict[str, Any]]`.
            - Assigned a Python `List[Dict[str, Any]]` directly via `setattr(character, 'inventory', updated_inventory)`.
            - Called `character_manager.mark_dirty(char_id_str, guild_id_str)` if available after inventory modifications, with logging and fallback save attempts if `mark_dirty` is missing.
    - **NLU Integration:**
        - Improved NLU logic in all commands to correctly use `nlu_identified_template_id` and `nlu_item_name_in_text` from `parse_player_action` results, with robust checks for entity existence in NLU response.
        - Added checks for `nlu_data_service` availability.
    - **Type Safety & Data Handling:**
        - Ensured character IDs and guild IDs are consistently handled as strings.
        - Handled potential `None` values from `getattr` and dictionary `.get()` calls.
        - Initialized `inventory_list_data` to an empty list if JSON parsing fails or if the attribute is `None`.
        - Ensured quantities for items are handled as floats.
        - Ensured `CoreGameRulesConfig` from `game_mngr.rule_engine.rules_config_data` (after `None` checks) is passed to relevant item manager methods.
    - **Error Messages & Responses:** Ensured user-facing messages from command results (e.g., `equip_result.get('message')`) are safely accessed using `.get()` with a default message.

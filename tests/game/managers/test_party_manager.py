...
class TestPartyManager(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.party_manager._deleted_parties: Dict[str, Set[str]] = set() # Initialize as set
    ...
    async def test_party_not_found_update_location(self):
        ...
        self.party_manager.mark_party_dirty.assert_not_called() # Should be assert_not_awaited for AsyncMock
    ...
    async def test_create_party_success(self):
        ...
        test_uuid_val = uuid.uuid4() # Generate a real UUID for the test
        ...
        self.assertEqual(created_party_object.id, str(test_uuid_val)) # Compare with str representation
    ...
    async def test_add_member_to_party_success(self):
        ...
        # Mock character needs to be awaitable if get_character_by_id is async
        ...
        self.party_manager.mark_party_dirty.assert_awaited_with(self.guild_id, self.test_party.id) # Use awaited_with
    ...
    async def test_add_member_already_in_party(self):
        ...
        result = await self.party_manager.add_member_to_party(self.guild_id, self.test_party.id, member_id, self.test_party.current_location_id)
        self.assertFalse(result)
        self.mock_character_manager.set_character_party_id.assert_not_awaited() # Use assert_not_awaited
    ...
    async def test_remove_member_leader_leaves_party_disbands(self):
        ...
        self.test_party.player_ids_list = [leader_id] # Only leader in party
        self.party_manager._parties.setdefault(self.guild_id, {})[self.test_party.id] = self.test_party
        ...
        self.assertTrue(result) # Result of remove_member should be true if disband is successful
        ...
        # set_character_party_id for the leader is called within disband_party in this scenario
    ...
    async def test_disband_party_success_as_leader(self):
        ...
        self.party_manager.mark_party_dirty.assert_not_awaited() # Not marked dirty, it's deleted
    ...
    async def test_save_state_saves_dirty_and_deletes_parties(self):
        ...
        # Check the actual data passed to upsert_party
        # The first argument to call_args is a tuple of positional args, the second is a dict of kwargs
        args, _ = self.mock_db_service.upsert_party.call_args
        self.assertEqual(args[0]['id'], dirty_party_id) # Assuming first arg is the party data dict
    ...
    async def test_load_state_for_guild_success(self):
        ...
        self.assertEqual(member_map.get(self.party_leader_id), "db_party_1") # player_ids_list[0] from party1_data
        self.assertEqual(member_map.get("ldr3"), "db_party_2") # leader_id from party2_data
    ...
    async def create_mock_character(self, player_id: str, location_id: str, status: str, actions_json: Optional[str] = "[]") -> MagicMock:
        ...
        char.id = player_id; char.name = f"Char_{player_id}"; char.current_location_id = location_id # Corrected attribute name
        char.current_game_status = status; char.collected_actions_json = actions_json # Corrected attribute name
    ...
    async def test_check_and_process_party_turn_not_all_ready(self):
        ...
        async def mock_get_char_side_effect(guild_id_arg: str, char_id_arg: str, session_arg: Any = None):
            # Ensure session_arg is optional and has a default
        ...
        # Assuming these methods are on db_service and game_manager.action_processor respectively
    ...
    async def test_check_and_process_party_turn_no_actions_data(self):
        ...
        self.test_party.player_ids_list = [str(char1.id)] # Ensure ID is string
        self.party_manager._parties.setdefault(self.guild_id, {})[self.party_id] = self.test_party
        ...
        mock_location_model = MagicMock() # Location doesn't need to be async for this
        ...
        # Mock the action_processor on the game_manager if it's accessed that way
        ...
        mock_discord_channel = AsyncMock(spec=discord.TextChannel) # Use spec for better mocking
        ...
        if hasattr(self.mock_db_service, 'execute_transactional_query_for_guild'): # Check if this is the method used
            self.mock_db_service.execute_transactional_query_for_guild.assert_any_call(self.guild_id, ANY, ('обработка', self.party_id))
            self.mock_db_service.execute_transactional_query_for_guild.assert_any_call(self.guild_id, ANY, ('сбор_действий', self.party_id))

        self.mock_game_manager.action_processor.process_party_actions.assert_awaited_once()
        # Check the actual arguments passed to process_party_actions
        ...
        self.assertEqual(char1.collected_actions_json, "[]") # Should be reset

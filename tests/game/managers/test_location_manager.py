...
DUMMY_LOCATION_TEMPLATE_DATA = {
    ...
    "exits": [{"direction":"north", "target_location_id":"tpl_north_region_entrance_id"}],
...
DUMMY_NORTH_REGION_TEMPLATE_DATA = {
    ...
    "exits": [{"direction":"south", "target_location_id":"tpl_world_center"}],
...
DUMMY_LOCATION_INSTANCE_FROM = {
    ...
    "exits": [{"direction":"north", "target_location_id":"instance_north_region_main_001"}],
...
DUMMY_LOCATION_INSTANCE_TO = {
    ...
    "exits": [{"direction":"south", "target_location_id":"instance_world_center_001"}],
...
class TestLocationManagerMoveEntity(BaseLocationManagerTest):
    async def asyncSetUp(self):
        ...
        self.mock_game_manager.party_manager.update_party_location = AsyncMock(return_value=True)
    ...
    async def test_successfully_moves_party(self):
        ...
        self.mock_game_manager.party_manager.update_party_location.assert_called_once()
    ...
    async def test_move_party_target_location_not_found(self, mock_get_instance_class_level: MagicMock):
        def mock_get_loc_inst_target_none(_slf, guild_id, loc_id) -> Optional[PydanticLocation]: # _slf for patched method
            if loc_id == self.to_location_id: return None
            if loc_id == self.from_location_id:
                # Access self.location_manager from the test instance
                cached_data = self.location_manager._location_instances.get(guild_id, {}).get(loc_id)
                return PydanticLocation.from_dict(cached_data) if cached_data else None
            return None

        mock_get_instance_class_level.side_effect = mock_get_loc_inst_target_none

        self.mock_game_manager.party_manager.update_party_location = AsyncMock()
        result = await self.location_manager.move_entity(
            guild_id=self.guild_id, entity_id=self.entity_id, entity_type="Party",
            from_location_id=self.from_location_id, to_location_id=self.to_location_id
        )
        self.assertFalse(result)
        self.mock_game_manager.party_manager.update_party_location.assert_not_called()

    async def test_move_party_party_manager_update_fails(self):
        self.mock_game_manager.party_manager.update_party_location = AsyncMock(return_value=False)
        result = await self.location_manager.move_entity(
            guild_id=self.guild_id, entity_id=self.entity_id, entity_type="Party",
            from_location_id=self.from_location_id, to_location_id=self.to_location_id
        )
...
class TestLocationManagerGetters(BaseLocationManagerTest):
    async def asyncSetUp(self):
        ...
        self.loc_data_1 = {
            ...
            "exits": [{"direction":"north", "target_location_id":"target1"}]
        }
        self.location_manager._location_instances = {
    ...
    async def test_get_location_by_static_id_from_db(self):
        ...
        for field in ["descriptions_i18n", "type_i18n", "state_variables", "exits",
                      "coordinates", "neighbor_locations_json", "generated_details_json",
                      "ai_metadata_json", "details_i18n", "tags_i18n", "atmosphere_i18n",
                      "features_i18n", "points_of_interest_json", "on_enter_events_json",
                      "on_exit_triggers", "on_enter_triggers", "initial_state"]:
        ...
        for field in ["template_id", "channel_id", "image_url", "moderation_request_id", "created_by_user_id"]:
    ...
    async def test_get_location_by_static_id_not_found_anywhere(self):
        ...
        with patch('bot.game.managers.location_manager.GuildTransaction', return_value=self.mock_db_service.get_session.return_value) as mock_gt_constructor_call:
        ...
        if self.location_manager._db_service:
             self.mock_db_service.get_session_factory.assert_called_once()
...
class TestLocationManagerProcessCharacterMove(BaseLocationManagerTest):
    async def asyncSetUp(self):
        ...
        self.pydantic_loc_from = PydanticLocation.from_dict({**DUMMY_LOCATION_INSTANCE_FROM, "id": self.loc_from_id, "guild_id": self.guild_id, "static_id": "static_from", "neighbor_locations_json": {self.loc_to_id: "path"}})
        self.pydantic_loc_to = PydanticLocation.from_dict({**DUMMY_LOCATION_INSTANCE_TO, "id": self.loc_to_id, "guild_id": self.guild_id, "static_id": "static_to"})

        self.mock_session_instance.get.side_effect = self._mock_session_get_side_effect

        # Patching methods on the class for the duration of the test method where needed
        # These side_effect functions now need to be standalone or static methods if used with class-level patch
        TestLocationManagerProcessCharacterMove._current_test_instance = self # Store self for side_effect
        ...
        self.mock_game_manager.rule_engine.get_rule = AsyncMock(return_value={"allow_leader_only_move": True, "teleport_all_members": True})

    _current_test_instance = None # Class variable to hold 'self' for static side_effect methods

    @staticmethod
    def _static_mock_lm_get_location_instance(_lm_instance, guild_id, loc_id) -> Optional[PydanticLocation]:
    ...
    @staticmethod
    async def _static_mock_lm_get_location_by_static_id(_lm_instance, guild_id, static_id_or_name, session=None) -> Optional[PydanticLocation]:
        test_self = TestLocationManagerProcessCharacterMove._current_test_instance
        name_i18n_to = test_self.pydantic_loc_to.name_i18n if test_self.pydantic_loc_to else {}
        name_i18n_from = test_self.pydantic_loc_from.name_i18n if test_self.pydantic_loc_from else {}
        if static_id_or_name == (test_self.pydantic_loc_to.static_id if test_self.pydantic_loc_to else None) or \
           (name_i18n_to and static_id_or_name == name_i18n_to.get("en")):
            return test_self.pydantic_loc_to
        if static_id_or_name == (test_self.pydantic_loc_from.static_id if test_self.pydantic_loc_from else None) or \
           (name_i18n_from and static_id_or_name == name_i18n_from.get("en")):
    ...
    async def test_move_single_character_success(self):
        ...
        self.mock_game_manager.game_log_manager.log_event.assert_awaited_once()
        self.mock_game_manager.location_interaction_service.process_on_enter_location_events.assert_awaited_once_with(
    ...
    async def test_move_fails_if_no_connection(self):
        if self.pydantic_loc_from: self.pydantic_loc_from.neighbor_locations_json = {}
        assert self.pydantic_loc_to is not None and self.pydantic_loc_to.static_id is not None
        with patch('bot.game.managers.location_manager.GuildTransaction', return_value=self.mock_db_service.get_session.return_value):
            result = await self.location_manager.process_character_move(self.guild_id, self.char_id, self.pydantic_loc_to.static_id)
        self.assertFalse(result)
        self.assertEqual(self.db_character.current_location_id, self.loc_from_id)
        self.mock_game_manager.location_interaction_service.process_on_enter_location_events.assert_not_awaited()

    async def test_move_fails_if_target_location_not_found(self):
        self.mocked_get_loc_static.return_value = None
        with patch('bot.game.managers.location_manager.GuildTransaction', return_value=self.mock_db_service.get_session.return_value):
    ...
    async def test_move_transaction_rollback_on_party_update_failure(self):
        ...
        async def mock_session_add_side_effect(instance):
            if isinstance(instance, discord.ext.commands.Cog): return
    ...
    async def test_move_party_member_not_leader_fails_if_rule_disallows(self):
        ...
        self.assertTrue(result)
        self.assertEqual(self.db_character.current_location_id, self.loc_to_id)
        self.assertEqual(self.db_party.current_location_id, self.loc_from_id)
        self.mock_game_manager.location_interaction_service.process_on_enter_location_events.assert_awaited_once_with(
            self.guild_id, self.char_id, "Character", self.loc_to_id
        )
    ...
    async def test_move_party_member_not_leader_succeeds_if_rule_allows_and_no_teleport(self):
        self.db_character.current_party_id = self.party_id
        self.db_party.leader_id = "other_char_leader"
        self.mock_game_manager.rule_engine.get_rule = AsyncMock(return_value={"allow_leader_only_move": False, "teleport_all_members": False})
        assert self.pydantic_loc_to is not None and self.pydantic_loc_to.static_id is not None
        with patch('bot.game.managers.location_manager.GuildTransaction', return_value=self.mock_db_service.get_session.return_value):
            result = await self.location_manager.process_character_move(self.guild_id, self.char_id, self.pydantic_loc_to.static_id)
        self.assertTrue(result)
...
class TestLocationManagerAICreation(BaseLocationManagerTest):
    ...
    async def test_create_location_instance_from_moderated_data_success_all_fields(self):
        ...
        moderated_loc_data = {
            ...
            "coordinates": {"x": 10}, "neighbor_locations_json": {"loc_north_id": {"en": "Path to the north"}},
            "generated_details_json": { "flora": {"en": "Glowing mushrooms"}}, "ai_metadata_json": { "prompt_version": "v1.2"},
            "details_i18n": {"en": "Whispering trees"}, "tags_i18n": {"en": "forest"},
            "atmosphere_i18n": {"en": "Ethereal"}, "features_i18n": {"en": "Shrine"},
            "exits": [{"direction":"north_actual_exit", "target_location_id":"some_other_place_id"}],
            ...
            "points_of_interest_json": [{"id": "poi1"}], "on_enter_events_json": [{"type": "message"}]
        }
    ...
    async def test_create_location_instance_from_moderated_data_generates_id_if_missing(self):
        ...
        moderated_loc_data = {"name_i18n": {"en": "Cave"}, "descriptions_i18n": {"en": "Needs ID."}, "template_id": "basic_cave_template" }
        context_with_request_id = {"request_id": request_id_val}
...
class TestLocationManagerTriggerHandling(BaseLocationManagerTest):
    async def asyncSetUp(self):
        ...
        self.location_instance_data_dict = {
        ...
        self.location_manager._location_instances = {self.guild_id: {self.location_instance_id: self.location_instance_data_dict}}

    @patch('bot.game.managers.location_manager.LocationManager.get_location_instance')
    async def test_handle_entity_arrival_success(self, mock_get_instance_class_level: MagicMock):
        mock_get_instance_class_level.return_value=PydanticLocation.from_dict(self.location_instance_data_dict)
        context_for_kwargs = {"guild_id": self.guild_id}
        await self.location_manager.handle_entity_arrival(
            location_id=self.location_instance_id, entity_id=self.entity_id,
            entity_type="Character", **context_for_kwargs
        )
        self.mock_game_manager.rule_engine.execute_triggers.assert_called_once_with(
    ...
    async def test_handle_entity_departure_success(self, mock_get_instance_class_level: MagicMock):
        ...
        await self.location_manager.handle_entity_departure(
            location_id=self.location_instance_id, entity_id=self.entity_id,
            entity_type="Party", **context_for_kwargs
        )
        self.mock_game_manager.rule_engine.execute_triggers.assert_called_once_with(
    ...
    async def test_handle_entity_arrival_no_template(self, mock_get_instance_class_level: MagicMock):
        ...
        self.mock_game_manager.rule_engine.execute_triggers.assert_not_called()
...
class TestLocationManager(BaseLocationManagerTest):
    ...
    async def test_init_manager(self):
        ...
        self.assertEqual(len(self.location_manager._location_templates), 2)
        self.assertIn("tpl_world_center", self.location_manager._location_templates)
        self.assertEqual(self.location_manager._location_instances, {self.guild_id: {}})
    ...
    def test_get_location_instance_success(self):
        ...
        self.location_manager._location_instances = { guild_id: { loc_id: loc_data } }
    ...
    def test_get_location_instance_wrong_guild(self):
        ...
        loc_data = {"id": loc_id, "guild_id": guild_id_correct, "name_i18n": {"en": "Loc Correct"}, "static_id": "s1"}
        self.location_manager._location_instances = {
    ...
    async def test_create_location_instance_success(self):
        ...
        self.location_manager._location_templates[template_id] = {
            "id": template_id, "name_i18n": {"en":"Base Template Main"},
            "description_i18n": {"en":"Base Desc Main"},
            "exits": [{"direction":"north", "target_location_id":"tpl_north_exit"}],
            "initial_state": {"temp_var": 1, "common_var": "template"}
        }
        self.location_manager._location_instances.setdefault(self.guild_id, {})
        self.location_manager._dirty_instances.setdefault(self.guild_id, set())
        ...
        with patch('uuid.uuid4', return_value=new_instance_id_obj):
            instance_dict_result = await self.location_manager.create_location_instance(
                ...
                instance_exits=[{"direction":"south", "target_location_id":"custom_south_exit"}],

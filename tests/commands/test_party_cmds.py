...
class TestPartyCommands(unittest.IsolatedAsyncioTestCase):
    ...
    def _reset_party_manager_mocks(self):
        ...
        self.mock_party_manager.create_party = AsyncMock(
            return_value=Party(id="new_party_123", name_i18n={"en": "Test Party"}, player_ids_list=[self.mock_player_character_db.id])
        )
        ...
        self.mock_party_manager.get_party_members = AsyncMock(return_value=[self.mock_player_character_db]) # Returns list of Pydantic/DB Characters
    ...
    async def test_party_create_successful(self):
        ...
        self.mock_party_manager.create_party.assert_awaited_once_with(
            ...
            leader_character_id=self.mock_player_character_db.id,
            party_name_i18n=expected_party_name_i18n
        )
        self.interaction.followup.send.assert_awaited_once()
        self.assertIn(f"Группа '{party_name}'", self.interaction.followup.send.call_args[0][0])
    ...
    async def test_party_create_already_in_party(self):
        self._reset_party_manager_mocks()
        self.mock_player_character_db.current_party_id = "existing_party_id" # Character is already in a party
        self.mock_character_manager.get_character.return_value = self.mock_player_character_db
        # Simulate PartyManager.create_party raising CharacterAlreadyInPartyError
        self.mock_party_manager.create_party.side_effect = CharacterAlreadyInPartyError("Already in party")

        await self.cog.cmd_party_create.callback(self.cog, self.interaction, name="New Party")
    ...
    async def test_party_join_successful(self):
        ...
        target_party_identifier = "PartyToJoinID"
        mock_target_party_db = MagicMock(spec=Party) # PartyManager.get_party returns SQLAlchemy model
        mock_target_party_db.id = target_party_identifier
        mock_target_party_db.name_i18n = {"en": "Party To Join"}
        mock_target_party_db.current_location_id = "location_A" # Same location as player
        self.mock_player_character_db.current_location_id = "location_A"

        self.mock_party_manager.get_party.return_value = mock_target_party_db
        self.mock_party_manager.join_party.return_value = True

        await self.cog.cmd_party_join.callback(self.cog, self.interaction, identifier=target_party_identifier)

        self.mock_party_manager.get_party.assert_awaited_once_with(str(self.interaction.guild.id), target_party_identifier)
        self.mock_party_manager.join_party.assert_awaited_once_with(
            guild_id=str(self.interaction.guild.id),
            character_id=self.mock_player_character_db.id,
    ...
    async def test_party_join_different_location_fail(self):
        self._reset_party_manager_mocks()
        self.mock_player_character_db.current_party_id = None
        self.mock_player_character_db.current_location_id = "location_A" # Player's location
        self.mock_character_manager.get_character.return_value = self.mock_player_character_db

        target_party_identifier = "FarAwayParty"
        mock_target_party_db = MagicMock(spec=Party)
        mock_target_party_db.id = target_party_identifier
        mock_target_party_db.name_i18n = {"en": "Far Away Party"}
        mock_target_party_db.current_location_id = "location_B" # Party's different location
        self.mock_party_manager.get_party.return_value = mock_target_party_db

        # Mock LocationManager to return display names for locations
    ...
    async def test_party_join_party_full(self):
        ...
        target_party_identifier = "FullHouseParty"
        mock_target_party_db = MagicMock(spec=Party)
        mock_target_party_db.id = target_party_identifier
        mock_target_party_db.name_i18n = {"en": "Full House Party"}
        mock_target_party_db.current_location_id = "location_A" # Same location
        self.mock_party_manager.get_party.return_value = mock_target_party_db
    ...
    async def test_party_leave_successful(self):
        self._reset_party_manager_mocks()
        self.mock_player_character_db.current_party_id = "current_party_id" # Character is in a party
        self.mock_character_manager.get_character.return_value = self.mock_player_character_db
        self.mock_party_manager.leave_party.return_value = True

        await self.cog.cmd_party_leave.callback(self.cog, self.interaction)
    ...
    async def test_party_leave_not_in_party(self):
        self._reset_party_manager_mocks()
        self.mock_player_character_db.current_party_id = None # Character not in a party
        self.mock_character_manager.get_character.return_value = self.mock_player_character_db
    ...
    async def test_party_disband_successful(self):
        ...
        await self.cog.cmd_party_disband.callback(self.cog, self.interaction)

        self.mock_party_manager.disband_party.assert_awaited_once_with(
            guild_id=str(self.interaction.guild.id),
            party_id="party_to_disband_id",
            disbanding_character_id=self.mock_player_character_db.id
        )
        self.interaction.followup.send.assert_awaited_once()

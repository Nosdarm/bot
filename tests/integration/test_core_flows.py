...
class BaseIntegrationTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.mock_game_manager.inventory_manager = AsyncMock(spec=InventoryManager) # Added this
...
class TestCharacterCreationFlow(BaseIntegrationTest):
    ...
    async def asyncSetUp(self):
        ...
        self.location_manager = LocationManager(db_service=self.mock_db_service, settings=self.mock_settings_dict)
        self.mock_game_manager.location_manager = self.location_manager

        self.item_manager = ItemManager(db_service=self.mock_db_service, settings=self.mock_settings_dict, location_manager=self.location_manager, rule_engine=self.mock_game_manager.rule_engine)
        self.mock_game_manager.item_manager = self.item_manager

        self.character_manager = CharacterManager(db_service=self.mock_db_service, settings=self.mock_settings_dict) # Simplified for now
        ...
        if self.default_start_location_instance_id:
            loc_instance_check = await self.location_manager.get_location(self.guild_id, self.default_start_location_instance_id) # Changed method and added await
    ...
    async def test_character_creation_and_initial_setup(self):
        ...
        created_char: Optional[Character] = await self.character_manager.create_new_character(
            guild_id=self.guild_id, user_id=discord_id_str,
            character_name={"en": char_name, "ru": char_name},
            language="en"
        )
        ...
        self.assertIsNotNone(created_char.location_id)
        self.assertEqual(created_char.location_id, self.default_start_location_instance_id)
        start_location_instance: Optional[Location] = await self.location_manager.get_location(self.guild_id, created_char.location_id) # Changed method and added await
...
class TestPlayerMovementFlow(BaseIntegrationTest):
    async def asyncSetUp(self):
        ...
        await super().asyncSetUp()
        self.rule_engine = RuleEngine(settings=self.mock_settings_dict)
        ...
        self.character_manager = CharacterManager(db_service=self.mock_db_service, settings=self.mock_settings_dict) # Simplified
        self.mock_game_manager.character_manager = self.character_manager
        ...
        with patch('uuid.uuid4', return_value=uuid.UUID('33333333-cccc-3333-cccc-333333333333')):
            self.test_char: Optional[Character] = await self.character_manager.create_new_character(
                guild_id=self.guild_id, user_id="char_mover_discord",
                character_name={"en":"CharMover"}, language="en"
            )
        self.assertIsNotNone(self.test_char)
        if self.test_char and self.loc_a_instance: self.assertEqual(self.test_char.location_id, self.loc_a_instance.id)
    ...
    async def test_player_moves_between_locations_with_triggers(self):
        ...
        updated_char = await self.character_manager.get_character(self.guild_id, self.test_char.id) # Changed to get_character_by_id
...
class TestItemInteractionFlow(BaseIntegrationTest):
    async def asyncSetUp(self):
        ...
        self.location_manager = LocationManager(self.mock_db_service, self.mock_settings_dict)
        self.mock_game_manager.location_manager = self.location_manager
        ...
        self.item_manager = ItemManager(self.mock_db_service, self.mock_settings_dict, self.location_manager, self.mock_game_manager.rule_engine)
        self.mock_game_manager.item_manager = self.item_manager
        await self.item_manager.load_item_templates() # Changed to await

        self.character_manager = CharacterManager(self.mock_db_service, self.mock_settings_dict) # Simplified
        self.mock_game_manager.character_manager = self.character_manager
        with patch('uuid.uuid4', return_value=uuid.UUID('char-int-item-flow---1111')):
            self.test_character = await self.character_manager.create_new_character(self.guild_id, "item_user_discord", character_name={"en":"ItemUser"}, language="en")
        ...
        with patch('uuid.uuid4', return_value=uuid.UUID('item-int-potion------1111')):
            self.item_in_location_dict = await self.item_manager.create_item(guild_id=self.guild_id, template_id=self.potion_template_id, quantity=1, location_id=self.test_location_instance.id if self.test_location_instance else None) # Changed method and quantity to float
    ...
    async def test_pick_up_item_from_location(self):
        ...
        update_success = await self.item_manager.update_item(self.guild_id, self.item_in_location.id, { "owner_id": self.test_character.id, "location_id": None })
        self.assertTrue(update_success)

        # Verify item is in character's inventory (using appropriate ItemManager method)
        char_items = await self.item_manager.get_character_inventory(self.guild_id, self.test_character.id)
        self.assertTrue(any(i.id == self.item_in_location.id for i in char_items))

        # Verify item is no longer in location's inventory
        loc_items_after = await self.item_manager.get_items_in_location(self.guild_id, self.test_location_instance.id)
    ...
    async def test_drop_item_to_location(self):
        ...
        with patch('uuid.uuid4', return_value=dropped_item_id_obj):
            char_item_dict = await self.item_manager.create_item(guild_id=self.guild_id, template_id=self.potion_template_id, quantity=1, owner_id=self.test_character.id)
        ...
        # Now, drop the item
        update_success = await self.item_manager.update_item(self.guild_id, char_item.id, { "owner_id": None, "location_id": self.test_location_instance.id })
        self.assertTrue(update_success)

        # Verify item is in location
        loc_items_after_drop = await self.item_manager.get_items_in_location(self.guild_id, self.test_location_instance.id)
        self.assertTrue(any(i.id == char_item.id for i in loc_items_after_drop))

        # Verify item is not in character's inventory
        char_items_after_drop = await self.item_manager.get_character_inventory(self.guild_id, self.test_character.id)
...
class TestCombatFlow(BaseIntegrationTest): # Added BaseIntegrationTest
    async def asyncSetUp(self):
        ...
        with patch('uuid.uuid4', return_value=uuid.UUID('hero-combat-flow---1111')):
            self.hero_char = await self.character_manager.create_new_character(self.guild_id, "hero_combat_discord", character_name={"en":"Hero"}, language="en", stats=json.dumps({"hp": 50, "max_health": 50, "attack": 10}))
        ...
        with patch('uuid.uuid4', return_value=uuid.UUID('vill-combat-flow---1111')):
            self.villain_char = await self.character_manager.create_new_character(self.guild_id, "villain_combat_discord", character_name={"en":"Villain"}, language="en", stats=json.dumps({"hp": 30, "max_health": 30, "defense": 2}))
    ...
    async def test_attack_and_health_update(self):
        ...
        initial_villain_hp_str = json.loads(self.villain_char.stats).get("hp", 0) if self.villain_char.stats else 0
        damage_dealt = 8 # Hero's attack (10) - Villain's defense (2 assumed from stats)
        new_villain_hp = initial_villain_hp_str - damage_dealt

        updated_char = await self.character_manager.update_character(self.guild_id, self.villain_char.id, {"stats": json.dumps({"hp": new_villain_hp})})
        self.assertTrue(updated_char)

        fetched_villain = await self.character_manager.get_character(self.guild_id, self.villain_char.id)
        self.assertIsNotNone(fetched_villain)
        if fetched_villain and fetched_villain.stats:
            self.assertEqual(json.loads(fetched_villain.stats).get("hp"), new_villain_hp)
    ...
    async def test_lethal_damage_and_death(self):
        ...
        initial_hero_hp_str = json.loads(self.hero_char.stats).get("hp", 0) if self.hero_char.stats else 0
        ...
        updated_char = await self.character_manager.update_character(self.guild_id, self.hero_char.id, {"stats": json.dumps({"hp": new_hero_hp})})
        self.assertTrue(updated_char)
        ...
        fetched_hero = await self.character_manager.get_character(self.guild_id, self.hero_char.id)
        self.assertIsNotNone(fetched_hero)
        if fetched_hero and fetched_hero.stats:
             self.assertEqual(json.loads(fetched_hero.stats).get("hp"), 0) # HP should be floored at 0

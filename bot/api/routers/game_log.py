...
from bot.database.models import GameLogEntry # Changed GameLog to GameLogEntry
...
async def create_game_log_entry(
    ...
    # The GameLogEntry model's id has a default=lambda: str(uuid.uuid4()), so no need to set it manually.
    # Timestamp is also auto-generated by server_default=func.now().
    db_log_entry = GameLogEntry( # Changed GameLog to GameLogEntry
        guild_id=guild_id,
...
async def list_game_log_entries(
    ...
    stmt = select(GameLogEntry).where(GameLogEntry.guild_id == guild_id) # Changed GameLog to GameLogEntry

    if event_type:
        stmt = stmt.where(GameLogEntry.event_type == event_type) # Changed GameLog to GameLogEntry
    if player_id:
        # This assumes player_id is directly on GameLogEntry or needs a JSON contains query if in involved_entities_ids
        # For simplicity, using the direct player_id field on GameLogEntry for now.
        stmt = stmt.where(GameLogEntry.player_id == player_id) # Changed GameLog to GameLogEntry
        # If filtering by involved_entities_ids:
        # from sqlalchemy.dialects.postgresql import JSONB
        # stmt = stmt.where(GameLogEntry.involved_entities_ids.cast(JSONB).contains([{"type": "player", "id": player_id}]))

    stmt = stmt.order_by(GameLogEntry.timestamp.desc()).offset(skip).limit(limit) # Changed GameLog to GameLogEntry

...
class AIResponseValidator:
    ...
    async def parse_and_validate_ai_response(
        ...
        MODEL_MAP: Dict[str, Any] = {
            ...
            GenerationType.NPC_PROFILE.value: GeneratedNpcProfile, # Removed _GENERATION
            GenerationType.QUEST_FULL.value: GeneratedQuestData, # Changed from QUEST_GENERATION to QUEST_FULL
            GenerationType.ITEM_PROFILE.value: GeneratedItemProfile, # Removed _GENERATION
        }
        ...
        if not pydantic_issues and model_instance_dict is not None and game_manager and game_manager.prompt_context_collector:
            try:
                ...
                if request_type_str == GenerationType.NPC_PROFILE.value: # Removed _GENERATION
                    semantic_issues.extend(await self._semantic_validate_npc_profile(model_instance_dict, game_terms, guild_id, game_manager))
                elif request_type_str == GenerationType.QUEST_FULL.value: # Changed from QUEST_GENERATION to QUEST_FULL
                ...
                elif request_type_str == GenerationType.ITEM_PROFILE.value: # Removed _GENERATION

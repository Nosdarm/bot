...
from .rules_schema import GameRules # Implicitly import others
# Game models for type hinting and reference
...
class AIResponseValidator:
    ...
    def _get_canonical_role_key(self, npc_data: Dict[str, Any], entity_info: str, errors: List[str], notifications: List[str]) -> Optional[str]:
        ...
        if isinstance(role_i18n, dict) and role_i18n:
            ...
            for lang, value in role_i18n.items():
    ...
    def _determine_status_and_moderation(self, errors: List[str], notifications: List[str]) -> tuple[str, bool]:
        requires_moderation = bool(errors)
    ...
    def validate_npc_block(self, npc_data: Dict[str, Any],
        ...
        entity_id_str = npc_data.get('id', npc_data.get('template_id'))
        ...
        # --- I18n, Role, Stats, Skills, Factions validation (condensed for brevity, logic unchanged) ---
        i18n_fields_to_check = ['name_i18n', 'backstory_i18n', 'role_i18n', 'personality_i18n', 'motivation_i18n', 'dialogue_hints_i18n', 'visual_description_i18n']
        ...
        if self._check_is_dict(stats_data, 'stats', entity_info, errors):
            ...
            for stat_key, stat_value in list(stats_data.items()):
                if stat_key not in valid_stats: errors.append(f"{entity_info}: Invalid stat name: '{stat_key}'.")
                elif not isinstance(stat_value, (int, float)): errors.append(f"{entity_info}: Stat '{stat_key}' value '{stat_value}' is not a number.")
                elif role_key and self.rules.character_stats_rules.stat_ranges_by_role:
                    ...
                    if role_specific_rules and role_specific_rules.stats:
                        ...
                        if stat_range_model:
                            ...
                            if not (min_val <= stat_value <= max_val):
                                corrected_value = max(min_val, min(stat_value, max_val)); stats_data[stat_key] = corrected_value
                                msg = f"{entity_info}: Stat '{stat_key}' value {stat_value} for role '{role_key}' was out of range ({min_val}-{max_val}). Clamped to {corrected_value}."
                                errors.append(msg); notifications.append(f"AUTO-CORRECT: {msg}")
                    elif role_key: notifications.append(f"{entity_info}: No stat range rules for role '{role_key}'. Skipping role-based value check for '{stat_key}'.")
        ...
        if self._check_is_dict(skills_data, 'skills', entity_info, errors):
            ...
            valid_skills = self.rules.skill_rules.valid_skills; skill_range = self.rules.skill_rules.skill_value_ranges
            min_skill, max_skill = skill_range.min, skill_range.max
            for skill_key, skill_value in list(skills_data.items()):
                if skill_key not in valid_skills: errors.append(f"{entity_info}: Invalid skill name: '{skill_key}'.")
                elif not isinstance(skill_value, (int, float)): errors.append(f"{entity_info}: Skill '{skill_key}' value '{skill_value}' is not a number.")
                elif not (min_skill <= skill_value <= max_skill):
                    corrected_value = max(min_skill, min(skill_value, max_skill)); skills_data[skill_key] = corrected_value
                    msg = f"{entity_info}: Skill '{skill_key}' value {skill_value} was out of range ({min_skill}-{max_skill}). Clamped to {corrected_value}."
                    errors.append(msg); notifications.append(f"AUTO-CORRECT: {msg}")
            ...
            if isinstance(faction_affiliations, list):
                if self.rules.faction_rules and self.rules.faction_rules.valid_faction_ids:
                    ...
                    for i, affiliation in enumerate(faction_affiliations):
                        aff_info = f"{entity_info}, Faction Affiliation index {i}"
                        if self._check_is_dict(affiliation, f"faction_affiliations[{i}]", entity_info, errors):
                            faction_id = affiliation.get('faction_id')
                            if faction_id:
                                if faction_id not in valid_faction_ids: errors.append(f"{aff_info}: Invalid faction_id '{faction_id}'.")
                            else: errors.append(f"{aff_info}: Missing 'faction_id'.")
                            rank_i18n = affiliation.get('rank_i18n')
                            if rank_i18n is not None and self._check_is_dict(rank_i18n, 'rank_i18n', aff_info, errors):
                                 errors.extend(self._validate_i18n_field_completeness(rank_i18n, 'rank_i18n', aff_info))
                elif self.rules.faction_rules is None: notifications.append(f"{entity_info}: Faction rules not defined. Skipping faction ID validation.")
            else: errors.append(f"{entity_info}: 'faction_affiliations' should be a list.")
    ...
    def validate_quest_block(self, quest_data: Dict[str, Any],
        ...
        # --- I18n, Referential Integrity, Structure, Rewards validation (condensed for brevity, logic unchanged) ---
        ...
        for field in top_level_i18n_fields:
            ...
            if field_value is not None and self._check_is_dict(field_value, field, entity_info, errors):
                errors.extend(self._validate_i18n_field_completeness(field_value, field, entity_info))

        prerequisites = quest_data.get('prerequisites')
        if isinstance(prerequisites, list):
            for p_id in prerequisites:
                if not isinstance(p_id, str) or p_id not in _existing_quest_ids: errors.append(f"{entity_info}: Prerequisite quest ID '{p_id}' invalid/not found.")

        connections = quest_data.get('connections')
        if isinstance(connections, dict):
            for c_type, c_ids in connections.items():
                if isinstance(c_ids, list):
                    for c_id in c_ids:
                        if not isinstance(c_id, str) or c_id not in _existing_quest_ids: errors.append(f"{entity_info}: Connected quest ID '{c_id}' ({c_type}) invalid/not found.")
                else: errors.append(f"{entity_info}: Connections for '{c_type}' must be a list.")
        ...
        if isinstance(npc_involvement, dict):
            for role, npc_id_val in npc_involvement.items():
                ...
                if not npc_ids_to_check and not isinstance(npc_id_val, (str, list)): errors.append(f"{entity_info}: NPC ID for role '{role}' invalid type.")
                for npc_id_item in npc_ids_to_check:
                    if not isinstance(npc_id_item, str) or npc_id_item not in _existing_npc_ids: errors.append(f"{entity_info}: Involved NPC ID '{npc_id_item}' ({role}) invalid/not found.")

        stages_field = quest_data.get('stages_i18n', quest_data.get('stages'))
        if not self._check_is_dict(stages_field, 'stages_i18n/stages', entity_info, errors) or not stages_field: errors.append(f"{entity_info}: Quest stages missing/invalid.")
        else:
            for s_key, s_data in cast(Dict[str, Any], stages_field).items():
                s_info = f"{entity_info}, Stage '{s_key}'"
                if not self._check_is_dict(s_data, f"Stage '{s_key}' data", s_info, errors): continue
                ...
                for field in stage_i18n_fields:
                    ...
                    if f_val is not None and self._check_is_dict(f_val, field, s_info, errors): errors.extend(self._validate_i18n_field_completeness(f_val, field, s_info))
                obj_field = s_data.get('objectives_i18n', s_data.get('objectives'))
                if not isinstance(obj_field, list) or not obj_field: errors.append(f"{s_info}: Objectives missing/invalid.")
                else:
                    for i, obj in enumerate(obj_field):
                        if not isinstance(obj, dict): errors.append(f"{s_info}, Objective {i}: Not a dictionary.")

        rewards_data = quest_data.get('rewards')
        if self._check_is_dict(rewards_data, 'rewards', entity_info, errors):
            rewards_data = cast(Dict[str, Any], rewards_data)
            xp = rewards_data.get('experience')
            if xp is not None:
                if not isinstance(xp, int) or xp < 0: errors.append(f"{entity_info}: XP reward '{xp}' must be non-negative int.")
                    ...
                    if not (xp_range.min <= xp <= xp_range.max):
                        corrected_xp = max(xp_range.min, min(xp, xp_range.max)); rewards_data['experience'] = corrected_xp
                        msg = f"{entity_info}: XP {xp} out of range ({xp_range.min}-{xp_range.max}). Clamped to {corrected_xp}."
                        errors.append(msg); notifications.append(f"AUTO-CORRECT: {msg}")
            ...
            if isinstance(item_rewards, list):
                for i, item_entry in enumerate(item_rewards):
                    ...
                    tid = item_entry if isinstance(item_entry, str) else item_entry.get('template_id') if isinstance(item_entry, dict) else None
                    if not tid or not isinstance(tid, str): errors.append(f"{item_info_ctx}: Invalid item entry or missing template_id.")
                    elif tid not in _existing_item_template_ids: errors.append(f"{item_info_ctx}: Item template_id '{tid}' not found.")
                    if isinstance(item_entry, dict) and item_entry.get('quantity') is not None and (not isinstance(item_entry['quantity'], int) or item_entry['quantity'] <=0):
                        errors.append(f"{item_info_ctx}: Quantity '{item_entry['quantity']}' invalid.")
            elif item_rewards is not None: errors.append(f"{entity_info}: 'items' in rewards must be a list.")
    ...
    def validate_item_block(self, item_data: Dict[str, Any],
        ...
        errors: List[str] = []
        notifications: List[str] = []
        entity_id_str = item_data.get('id', item_data.get('template_id'))
        ...
        # --- Template ID, I18n, Type, Price, Properties validation (condensed for brevity, logic unchanged) ---
        template_id = item_data.get('template_id')
        if not template_id: errors.append(f"{entity_info}: Missing 'template_id'.")
        ...
        for field in item_i18n_fields:
            ...
            if f_val is not None and self._check_is_dict(f_val, field, entity_info, errors):
                errors.extend(self._validate_i18n_field_completeness(f_val, field, entity_info))

        item_type = item_data.get('type'); item_type_validated = False
        if item_type:
            if isinstance(item_type, str):
                if self.rules.item_rules and self.rules.item_rules.valid_item_types:
                    if item_type not in self.rules.item_rules.valid_item_types: errors.append(f"{entity_info}: Invalid item type: '{item_type}'.")
                    else: item_type_validated = True
                else: notifications.append(f"{entity_info}: No valid_item_types rule. Skipping type validation for '{item_type}'.")
            else: errors.append(f"{entity_info}: Item 'type' must be string.")

        price = item_data.get('price')
        if price is not None:
            if not isinstance(price, (int, float)) or price < 0: errors.append(f"{entity_info}: Price '{price}' must be non-negative number.")
            else:
                if self.rules.item_rules and self.rules.item_rules.price_ranges_by_type:
                    if item_type and item_type_validated:
                        item_rarity = item_data.get('rarity', 'common').lower()
                        price_cat_rules = self.rules.item_rules.price_ranges_by_type.get(item_type)
                        if price_cat_rules and price_cat_rules.prices:
                            price_detail = price_cat_rules.prices.get(item_rarity)
                            if price_detail:
                                if not (price_detail.min <= price <= price_detail.max):
                                    clamped_price = max(price_detail.min, min(price, price_detail.max)); item_data['price'] = clamped_price
                                    msg = f"{entity_info}: Price {price} for type '{item_type}' rarity '{item_rarity}' out of range ({price_detail.min}-{price_detail.max}). Clamped to {clamped_price}."
                                    errors.append(msg); notifications.append(f"AUTO-CORRECT: {msg}")
                            else: notifications.append(f"{entity_info}: No price rules for rarity '{item_rarity}' of type '{item_type}'.")
                        else: notifications.append(f"{entity_info}: No price categories for type '{item_type}'.")
                    elif item_type and not item_type_validated: notifications.append(f"{entity_info}: Item type '{item_type}' invalid. Skipping price range validation.")
                    else: notifications.append(f"{entity_info}: Item type not specified. Skipping type/rarity price validation.")
                else: notifications.append(f"{entity_info}: No price_ranges_by_type rule. Skipping price range validation.")

        if item_type_validated:
            if item_type == 'weapon' and 'damage' not in item_data: notifications.append(f"{entity_info}: Weapon missing 'damage' field (soft check).")
            if item_type == 'potion' and 'effect' not in item_data: notifications.append(f"{entity_info}: Potion missing 'effect' field (soft check).")
    ...
    def validate_ai_response(self, ai_json_string: str, expected_structure: str,
        ...
        validator_func: Optional[callable] = None # Make type checker happy
        is_list = False
        ...
        context_args = {
            "existing_quest_ids": existing_quest_ids, "existing_npc_ids": existing_npc_ids,
            ...
            if not isinstance(parsed_data, list):
                ...
                for i, item_data_uncast in enumerate(parsed_data):
                    if not isinstance(item_data_uncast, dict):
                        # Add a placeholder error entity if an item in the list is not a dict
                        err_msg = f"Item at index {i} for '{expected_structure}' is not a dictionary, skipping validation for this item."
                        global_errors.append(err_msg) # Also as a global error
                        entities.append({"entity_id": None, "type": expected_structure.split('_')[-1][:-1], # "npc", "quest", "item"
        ...
        overall_status = "success" # Default
        if global_errors:

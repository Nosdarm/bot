...
if TYPE_CHECKING:
    ...
    from bot.database.models.config_related import GuildConfig # Corrected import path
    ...
    async def request_content_generation(
        ...
        prepare_prompt_args: Dict[str, Any] = {
            ...
            "location_id": str(context_params.get("location_id")) if context_params.get("location_id") else \
                           (str(generation_context.primary_location_details.get("id")) if generation_context.primary_location_details and isinstance(generation_context.primary_location_details.get("id"), (str, int)) else "default_world_space"),
        ...
        pending_generation_record: Optional[PendingGeneration] = None # Removed direct instantiation
    ...
    async def process_approved_generation( # ... (signature as before) ...
        ...
        try:
            async with GuildTransaction(session_factory_callable(), guild_id) as session: # type: ignore[operator]
                ...
                # Explicitly get value from enum for comparison if record.status is an enum instance
                current_status_value = record.status.value if isinstance(record.status, Enum) else record.status
                if current_status_value != PendingStatus.APPROVED.value:
                    logger.warning(f"PG {record_id_str} not APPROVED. Status: {current_status_value}"); return False
                ...
                if not record.parsed_data_json or not isinstance(record.parsed_data_json, dict):
                    ...
                    await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, parsed_data_json={"error": "No parsed data."})
                    return False

                request_type_val: Optional[GenerationType] = None
                # record.request_type should be an instance of GenerationType enum if loaded from DB correctly
                if isinstance(record.request_type, GenerationType):
                    request_type_val = record.request_type
                elif isinstance(record.request_type, str): # If it was stored as string and loaded as string
                    try:
                        request_type_val = GenerationType(record.request_type) # Try to convert string value to Enum
                    except ValueError:
                        logger.error(f"Invalid string value for request_type '{record.request_type}' in PG {record_id_str}.")

                if not request_type_val: # Checks if request_type_val is None
                    logger.error(f"Invalid or missing request_type for PG {record_id_str}. Got: {record.request_type}");
                    await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, validation_issues_json=[{"error": f"Invalid type {record.request_type}."}])
                    return False

...
if TYPE_CHECKING:
    ...
    from bot.ai.ai_response_validator import AIResponseValidator # Kept for type hinting
    from bot.game.managers.game_manager import GameManager
    from sqlalchemy.ext.asyncio import AsyncSession
    # from bot.ai.ai_response_validator import parse_and_validate_ai_response # This is a function, not a type
# Moved out for runtime use:
...
class AIGenerationManager:
    ...
    async def request_content_generation(
        ...
        # Pass all context_params directly, get_full_context should handle what it needs.
        generation_context = await self.prompt_context_collector.get_full_context(
            guild_id=guild_id,
            **context_params # Pass all context parameters
        )
        ...
        # Pass all prompt_params directly, prepare_ai_prompt should handle what it needs.
        # Remove target_languages as it's handled separately.
        prompt_params_for_generator = {k: v for k, v in prompt_params.items() if k != "target_languages"}

        final_prompt_str = await self.multilingual_prompt_generator.prepare_ai_prompt(
            generation_type_str=request_type.value, # string representation of enum
            context_data=generation_context,
            target_character_id=str(context_params.get("character_id")) if context_params.get("character_id") else None,
            target_languages=target_languages_list,
            **prompt_params_for_generator # Pass remaining prompt parameters
        )

        logger.info(f"AIGenerationManager: Using placeholder AI response for '{request_type.value}'.")
        raw_ai_output_text = "" # Placeholder
        if request_type == GenerationType.LOCATION_DETAILS:
            raw_ai_output_text = json.dumps({ # Ensure it's text
        ...
        parsed_data, validation_issues = await parse_and_validate_ai_response( # Use the imported function
        ...
        pending_generation_record: Optional[PendingGeneration] = None # Explicit type
        session_to_use: Optional[AsyncSession] = session

        async def create_record_internal(current_session_internal: AsyncSession):
            nonlocal pending_generation_record # Ensure we modify the outer scope variable
            pending_generation_record = await self.pending_generation_crud.create_pending_generation(
                ...
                request_params_json=context_params, raw_ai_output_text=raw_ai_output_text, parsed_data_json=parsed_data,
        ...
        if session_to_use:
            ...
            try:
                get_session_factory_method = getattr(self.db_service, 'get_session_factory', None)
                if not callable(get_session_factory_method):
                    logger.error("DBService does not have a callable 'get_session_factory' method.")
                    return None # Or raise an appropriate error
                async with GuildTransaction(get_session_factory_method, guild_id) as new_session_ctx:
        ...
        if pending_generation_record and hasattr(pending_generation_record, 'id') and pending_generation_record.id:
            ...
            if current_status == PendingStatus.PENDING_MODERATION and callable(send_notification_method):
                ...
                if db_service_gm and hasattr(db_service_gm, 'models') and hasattr(db_service_gm.models, 'GuildConfig') and \
                   hasattr(db_service_gm, 'get_entity_by_pk') and callable(getattr(db_service_gm, 'get_entity_by_pk')):
                    get_entity_pk_method = getattr(db_service_gm, 'get_entity_by_pk')
                    guild_config_result = await get_entity_pk_method(db_service_gm.models.GuildConfig, guild_id) # type: ignore[attr-defined]

                if guild_config_result: # This should be GuildConfig instance
                    ...
                    if notification_channel_id_val:
                        ...
                        try:
                            await send_notification_method(int(str(notification_channel_id_val)), msg) # Ensure int
                        except ValueError:
            ...
            return None # Explicitly return None if record creation failed or record has no ID
    ...
    async def request_location_generation(
        ...
        return await self.request_content_generation(
            ...
            created_by_user_id=str(created_by_user_id) if created_by_user_id else None, # Ensure string or None
            session=session
        )
    ...
    async def process_approved_generation(
        ...
        try:
            ...
            async with GuildTransaction(get_session_factory_method, guild_id) as session:
                record = await self.pending_generation_crud.get_pending_generation_by_id(session, pending_generation_id, guild_id)
                if not record or not hasattr(record, 'id'): # Ensure record and record.id exist
                    logger.error(f"PendingGeneration record {pending_generation_id} not found or invalid for guild {guild_id}."); return False

                record_id_str = str(record.id) # Use this for all subsequent calls requiring record.id
                ...
                if not record_parsed_data_json:
                    ...
                    await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, "No parsed data.")
                    return False

                request_type_val_str = getattr(record, 'request_type', None)
                request_type_val: Optional[GenerationType] = None
                if isinstance(request_type_val_str, str):
                    try: request_type_val = GenerationType[request_type_val_str.upper()]
                    except KeyError: pass # Handled below
                elif isinstance(request_type_val_str, GenerationType): # Already enum
                ...
                if not request_type_val:
                    ...
                    await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, "Invalid request type."); return False
                ...
                if request_type_val == GenerationType.LOCATION_DETAILS:
                    ...
                    if isinstance(parsed_data, dict):
                        try:
                            ...
                            await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, f"Parse AI data error: {str(e)[:100]}"); return False
                    else:
                        logger.error(f"parsed_data for LOCATION_DETAILS {record_id_str} is not a dict.")
                        await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, "Parsed data not a dict."); return False


                    if ai_location_data: # ai_location_data is now GeneratedLocationContent
                        ...
                        if loc_static_id and isinstance(loc_static_id, str): # Ensure static_id is string
                        ...
                        if existing_loc:
                            ...
                            loc_to_persist = Location(guild_id=guild_id, id=str(uuid.uuid4()), static_id=str(loc_static_id) if loc_static_id else None) # Ensure static_id is str or None
                            logger.info(f"Creating new location {loc_to_persist.id}")

                        loc_to_persist.name_i18n = ai_location_data.name_i18n if isinstance(ai_location_data.name_i18n, dict) else {}
                        loc_to_persist.descriptions_i18n = ai_location_data.atmospheric_description_i18n if isinstance(ai_location_data.atmospheric_description_i18n, dict) else {}
                        ...
                        loc_to_persist.coordinates = ai_location_data.coordinates_json if isinstance(ai_location_data.coordinates_json, dict) else None
                        loc_to_persist.details_i18n = ai_location_data.generated_details_json if isinstance(ai_location_data.generated_details_json, dict) else None
                        loc_to_persist.ai_metadata_json = ai_location_data.ai_metadata_json if isinstance(ai_location_data.ai_metadata_json, dict) else None


                        pois_list = ai_location_data.points_of_interest if isinstance(ai_location_data.points_of_interest, list) else []
                        loc_to_persist.points_of_interest_json = [poi.model_dump() for poi in pois_list if isinstance(poi, POIModel) and hasattr(poi, 'model_dump')]

                        conns_list = ai_location_data.connections if isinstance(ai_location_data.connections, list) else []
                        loc_to_persist.neighbor_locations_json = [conn.model_dump() for conn in conns_list if isinstance(conn, ConnectionModel) and hasattr(conn, 'model_dump')]

                        # Mark all JSONB fields for modification explicitly
                        ...
                        try:
                            ...
                            await session.flush() # Important to get ID if new
                            loc_id_after_flush = getattr(loc_to_persist, 'id', None)
                            if loc_id_after_flush is not None:
                                record.entity_id = str(loc_id_after_flush) # Save persisted entity ID to the PG record
                                ...
                                if created_by_user_id_val and location_interaction_service and \
                                   ...
                                   callable(getattr(location_interaction_service, 'process_on_enter_location_events')) and \
                            ...
                            else: # Should not happen if flush is successful and ID is generated
                            ...
                            application_success = False
                            await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, f"DB merge fail: {str(e_merge)[:100]}"); return False
                        ...
                        if application_success and persisted_location_id and isinstance(ai_location_data.connections, list):
                            ...
                            for conn_data in ai_location_data.connections:
                                ...
                                if not neighbor_id or str(neighbor_id) == persisted_location_id: continue # Ensure str comparison
                                neighbor_loc_res = await session.get(Location, str(neighbor_id)) # Ensure str ID
                                if neighbor_loc_res and str(neighbor_loc_res.guild_id) == guild_id: # Ensure str comparison
                                    ...
                                    if not any(isinstance(c, dict) and c.get("to_location_id") == persisted_location_id for c in current_neighbors):
                        ...
                        initial_npcs_json_val = getattr(ai_location_data, 'initial_npcs_json', None) if hasattr(ai_location_data, 'initial_npcs_json') else None
                        if application_success and persisted_location_id and isinstance(initial_npcs_json_val, list) and \
                           ...
                           hasattr(self.game_manager.npc_manager, 'spawn_npc_in_location') and callable(getattr(self.game_manager.npc_manager, 'spawn_npc_in_location')):
                            ...
                            for npc_prof_data_any in initial_npcs_json_val:
                                ...
                                npc_state.pop('faction_affiliations', None) # Remove original to avoid conflict
                                npc_template_id_str = str(npc_prof_data.template_id) if npc_prof_data.template_id else "unknown_npc_template"
                                created_npc = await self.game_manager.npc_manager.spawn_npc_in_location(guild_id, persisted_location_id, npc_template_id_str, False, npc_state, session)
                                if created_npc and hasattr(created_npc, 'id') and created_npc.id: new_npc_ids.append(str(created_npc.id))
                                else: application_success = False; logger.error(f"Failed to spawn NPC {npc_template_id_str} for PG {record_id_str}."); await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, "NPC spawn fail."); break
                            if application_success and new_npc_ids:
                                ...
                                if loc_for_npc_update_res:
                                    ...
                                    current_npc_ids_list = loc_for_npc_update.npc_ids if isinstance(loc_for_npc_update.npc_ids, list) else []
                                    for nid_str in new_npc_ids:
                                        if nid_str not in current_npc_ids_list: current_npc_ids_list.append(nid_str)
                                    loc_for_npc_update.npc_ids = current_npc_ids_list
                        ...
                        initial_items_json_val = getattr(ai_location_data, 'initial_items_json', None) if hasattr(ai_location_data, 'initial_items_json') else None
                           ...
                           hasattr(self.game_manager.item_manager, 'create_item_instance') and callable(getattr(self.game_manager.item_manager, 'create_item_instance')):
                            ...
                            if not loc_for_item_updates_res: application_success = False; logger.critical(f"Location {persisted_location_id} vanished for item add for PG {record_id_str}.")
                                ...
                                for item_entry in initial_items_json_val:
                                    ...
                                    item_qty_float = 1.0
                                    try: item_qty_float = float(item_qty_any)
                                    except (ValueError, TypeError): pass
                                    poi_id_str = str(poi_id_any) if poi_id_any else None

                                    created_item = await self.game_manager.item_manager.create_item_instance(guild_id, item_tpl_id_str, item_qty_float, persisted_location_id, "location", persisted_location_id, {"is_in_poi_id": poi_id_str} if poi_id_str else None, session)
                                    ...
                                    elif created_item: items_changed_loc = True # Item created, might not be in POI
                                    else: application_success = False; logger.error(f"Failed item spawn {item_tpl_id_str} for PG {record_id_str}."); await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id,"Item spawn fail."); break
                                if items_changed_loc and loc_for_item_updates: # Check loc_for_item_updates again as it's from a session.get
                                    if loc_for_item_updates.points_of_interest_json is not None: flag_modified(loc_for_item_updates, "points_of_interest_json")
                                    # Inventory on location model might not exist or be handled differently than POI items.
                                    # If loc_for_item_updates.inventory is a field, it should also be flagged.
                                    if hasattr(loc_for_item_updates, 'inventory') and loc_for_item_updates.inventory is not None: flag_modified(loc_for_item_updates, "inventory")
                    ...
                    record.entity_id = f"simulated_{request_type_val.value.lower()}_{record_id_str[:8]}" # Example entity ID
                    flag_modified(record, "entity_id")
                    application_success = True
                else:
                    logger.warning(f"No specific persistence logic for type '{request_type_val.value}' for PG {record_id_str}.")
                    application_success = False
                    await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, f"No app logic for {request_type_val.value}.")
                    return False

                if application_success:
                    await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLIED, guild_id, moderator_user_id, str(getattr(record, 'moderator_notes', None)))
                    ...
                    record_status_val_after = getattr(record, 'status', None)
                    if record_status_val_after != PendingStatus.APPLICATION_FAILED.value: # Compare with .value
                         await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, (str(getattr(record, 'moderator_notes', None) or "")) + " | App persistence failed.")
                    return False
        except Exception as e:
            logger.exception(f"Unexpected error in process_approved_generation for {pending_generation_id}")
            try:
                get_session_factory_method_err = getattr(self.db_service, 'get_session_factory', None)
                if not callable(get_session_factory_method_err):
                    logger.error("DBService does not have a callable 'get_session_factory' method for error status update in exception block.")
                    return False
                async with GuildTransaction(get_session_factory_method_err, guild_id) as error_session:

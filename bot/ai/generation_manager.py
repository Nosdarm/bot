...
# Ensure parse_and_validate_ai_response is imported for runtime if used directly
# from bot.ai.ai_response_validator import parse_and_validate_ai_response
# It's already imported below, so this is fine.
...
class AIGenerationManager:
    ...
    async def request_content_generation(
        ...
        if not target_languages_list:
            ...
            if "en" not in target_languages_list:
                target_languages_list.append("en")
        target_languages_list = sorted(list(set(target_languages_list)))

        # Corrected parameter passing for prepare_ai_prompt
        # Assuming context_params might contain some direct args for prepare_ai_prompt
        # and prompt_params contains others, especially additional_request_params.

        # Start with parameters directly from context_params if they match prepare_ai_prompt's signature
        ...
        # Additional parameters that might be in prompt_params or context_params
        # This is a more direct way to pass what prepare_ai_prompt expects, if its signature is known.
        # For now, we'll pass most through additional_request_params as before, but ensure known ones are separate.
        additional_request_params_for_prompt: Dict[str, Any] = {
            ...
            "target_languages": target_languages_list, # This is now correctly a list of strings
            "generation_type_str": request_type.value, # Added generation_type_str
            "context_data": generation_context, # Passed the full context
            **{k: v for k, v in prompt_params.items() if k not in ["target_character_id", "target_languages", "specific_task_instruction"]} # Other prompt_params
        }
        additional_request_params_for_prompt = {k: v for k, v in additional_request_params_for_prompt.items() if v is not None}

        prepare_prompt_args["additional_request_params"] = additional_request_params_for_prompt
        ...
        async def create_record_internal(current_session_internal: AsyncSession):
            ...
            pending_generation_record = await self.pending_generation_crud.create_pending_generation(
                ...
                request_params_json=context_params,
        ...
        if session_to_use:
            ...
            try:
                session_factory_method: Any = getattr(self.db_service, 'get_session_factory', None)
                if not callable(session_factory_method):
                    logger.error("DBService does not have a callable 'get_session_factory' method.")
                    return None
                async with GuildTransaction(session_factory_method(), guild_id) as new_session_ctx: # Call session_factory_method
        ...
        if pending_generation_record and hasattr(pending_generation_record, 'id') and pending_generation_record.id:
            ...
            if current_status == PendingStatus.PENDING_MODERATION and callable(send_notification_method):
                ...
                if db_service_gm and hasattr(db_service_gm, 'get_entity_by_pk') and callable(getattr(db_service_gm, 'get_entity_by_pk')):
                    from bot.database.models.guild_config import GuildConfig
                    guild_config_result = await db_service_gm.get_entity_by_pk(GuildConfig, guild_id)


                if guild_config_result and isinstance(guild_config_result, GuildConfig): # Check instance type
    ...
    async def process_approved_generation(
        ...
        try:
            ...
            async with GuildTransaction(session_factory_method(), guild_id) as session: # Call session_factory_method
                ...
                record_status_val = record.status
                ...
                record_parsed_data_json = record.parsed_data_json
                ...
                request_type_val_str = record.request_type
                ...
                if not request_type_val:
                    ...
                    await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, validation_issues_json=[{"error": "Invalid request type."}]); return False # type: ignore[call-arg]
                ...
                if request_type_val == GenerationType.LOCATION_DETAILS:
                    ...
                    if isinstance(parsed_data, dict):
                        try:
                            ...
                            await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, parsed_data_json={"error": f"Parse AI data error: {str(e)[:100]}"}); return False # type: ignore[call-arg]
                    else:
                        logger.error(f"parsed_data for LOCATION_DETAILS {record_id_str} is not a dict.")
                        await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, parsed_data_json={"error": "Parsed data not a dict."}); return False # type: ignore[call-arg]
                    ...
                    if ai_location_data:
                        ...
                        try:
                            ...
                            await session.flush()
                            loc_id_after_flush = loc_to_persist.id
                            ...
                            application_success = False
                            await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, parsed_data_json={"error": f"DB merge fail: {str(e_merge)[:100]}"}); return False # type: ignore[call-arg]
                            ...
                            for conn_data in ai_location_data.connections:
                                ...
                                if neighbor_loc_res and str(neighbor_loc_res.guild_id) == guild_id:
                                    ...
                                    if not any(isinstance(c, dict) and c.get("to_location_id") == persisted_location_id for c in current_neighbors_list):
                                        ...
                                        neighbor_loc.neighbor_locations_json = current_neighbors_list
                           ...
                           hasattr(self.game_manager.npc_manager, 'spawn_npc_in_location') and callable(getattr(self.game_manager.npc_manager, 'spawn_npc_in_location')):
                            ...
                            if application_success and new_npc_ids:
                                ...
                                if loc_for_npc_update_res:
                                    ...
                                    loc_for_npc_update.npc_ids = current_npc_ids_list_val
                           ...
                           hasattr(self.game_manager.item_manager, 'create_item_instance') and callable(getattr(self.game_manager.item_manager, 'create_item_instance')):
                            ...
                            if not loc_for_item_updates_res: application_success = False; logger.critical(f"Location {persisted_location_id} vanished for item add for PG {record_id_str}.")
                                ...
                                for item_entry in initial_items_json_val:
                                    ...
                                    created_item = await self.game_manager.item_manager.create_item_instance(
                                        ...
                                        db_session=session, is_temporary=False
                                    )
                                ...
                                if items_changed_loc and loc_for_item_updates:
                                    ...
                                    if hasattr(loc_for_item_updates, 'inventory_json') and loc_for_item_updates.inventory_json is not None: flag_modified(loc_for_item_updates, "inventory_json")
                    ...
                    application_success = False
                    await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, validation_issues_json=[{"error": f"No app logic for {request_type_val.value}."}]); return False # type: ignore[call-arg]

                if application_success:
                    await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLIED, guild_id, moderator_user_id, moderator_notes=str(getattr(record, 'moderator_notes', None))) # type: ignore[call-arg]
                    ...
                    record_status_val_after = record.status
                    if record_status_val_after != PendingStatus.APPLICATION_FAILED.value:
                         await self.pending_generation_crud.update_pending_generation_status(session, record_id_str, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, moderator_notes=(str(getattr(record, 'moderator_notes', None) or "")) + " | App persistence failed.") # type: ignore[call-arg]
                    return False
        except Exception as e:
            logger.exception(f"Unexpected error in process_approved_generation for {pending_generation_id}")
            try:
                session_factory_method_err: Any = getattr(self.db_service, 'get_session_factory', None) # Renamed to avoid conflict
                if not callable(session_factory_method_err):
                    logger.error("DBService does not have a callable 'get_session_factory' method for error status update in exception block.")
                    return False
                async with GuildTransaction(session_factory_method_err(), guild_id) as error_session: # Call session_factory_method_err
                    await self.pending_generation_crud.update_pending_generation_status(error_session, pending_generation_id, PendingStatus.APPLICATION_FAILED, guild_id, moderator_user_id, moderator_notes=f"App fail exception: {str(e)[:100]}") # type: ignore[call-arg]

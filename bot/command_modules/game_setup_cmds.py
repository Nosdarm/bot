...
if TYPE_CHECKING:
    ...
    from bot.game.models.character import Character # Added for return type hint
...
class GameSetupCog(commands.Cog, name="Game Setup"):
    ...
    async def cmd_start_new_character(self, interaction: Interaction, character_name: str, player_language: Optional[str] = None):
        ...
        try:
            ...
            # The method signature for start_new_character_session in GameManager is (self, user_id: int, guild_id: str, character_name: str)
            # It does not take discord_user_name, channel_id, selected_language directly like this.
            # This call needs to be updated based on the actual signature implemented in GameManager.
            # For now, assuming the subtask to implement start_new_character_session will define its signature.
            # Based on the provided signature for GameManager.start_new_character_session:
            # (self, user_id: int, guild_id: str, character_name: str) -> Optional["Character"]
            # The call here needs adjustment. The current subtask is to fix game_setup_cmds.py access,
            # and then implement the method in GameManager.
            # The success, message tuple return is also not matching.
            # Let's adjust the call to the specified signature and handle the Character return.
            new_character = await game_mngr.start_new_character_session(
                user_id=interaction.user.id,
                guild_id=str(interaction.guild_id),
                character_name=character_name
                # selected_language and discord_user_name would need to be handled differently,
                # e.g. by setting language on the character object afterwards if create_character doesn't take it.
            )

            if new_character:
                # If selected_language was provided and Character model has selected_language field
                if player_language and hasattr(new_character, 'selected_language'):
                    if game_mngr.character_manager and hasattr(game_mngr.character_manager, 'save_character_field'):
                ...
                char_name_display = getattr(new_character, 'name', character_name) # Fallback to input name
                if hasattr(new_character, 'name_i18n') and isinstance(new_character.name_i18n, dict):
                    char_name_display = new_character.name_i18n.get(effective_language, char_name_display)

                await interaction.followup.send(f"Персонаж '{char_name_display}' успешно создан! Язык: {effective_language}.", ephemeral=True)
            else:
                await interaction.followup.send(f"Не удалось создать персонажа '{character_name}'.", ephemeral=True)
    ...
    async def cmd_set_bot_language(self, interaction: Interaction, language_code: str):
        ...
        bot_instance = self.bot # type: RPGBot
        if not hasattr(bot_instance, 'game_manager') or bot_instance.game_manager is None:
            await interaction.response.send_message("GameManager is not available. Please try again later or contact an admin.", ephemeral=True)
            return
        game_mngr: "GameManager" = bot_instance.game_manager

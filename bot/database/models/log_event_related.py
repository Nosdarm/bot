...
# The PendingGeneration model (and its enums GenerationType, PendingStatus)
# is now defined in bot/models/pending_generation.py
# and should be imported via bot.database.models.__init__.py if needed by Alembic.
# This definition is removed to prevent 'Table already defined' error.
#
...
#     guild_id = Column(String, ForeignKey('guild_configs.guild_id', ondelete='CASCADE'), nullable=False, index=True)
#
#     request_type = Column(String, nullable=False, index=True) # Old: SAEnum(GenerationType, name="generation_type_enum")
...
#
#     status = Column(String, nullable=False, default="pending_validation", index=True) # Old: SAEnum(PendingStatus, name="pending_status_enum")
#
#     created_by_user_id = Column(String, nullable=True)
#     created_at = Column(TIMESTAMP(timezone=True), server_default=func.now(), nullable=False) # Was DateTime(timezone=True)
#
#     moderated_by_user_id = Column(String, nullable=True)
#     moderated_at = Column(TIMESTAMP(timezone=True), nullable=True) # Was DateTime(timezone=True)
#     moderator_notes_i18n = Column(JSONB, nullable=True) # Was Text (moderator_notes)
#
#     # Relationship to GuildConfig (optional, but good practice)
#     # guild = relationship("GuildConfig") # This would need GuildConfig to be imported
#
#     __table_args__ = (
#         Index('idx_pendinggeneration_guild_status', 'guild_id', 'status'), # Old: idx_pending_generation_guild_status_type
#     )
#
#     def __repr__(self):

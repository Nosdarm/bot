...
class CharacterActionProcessor:
    ...
    async def process_action_from_request(self, action_request: ActionRequest, character: Character, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not rules_config and self._game_manager and hasattr(self._game_manager, 'get_core_rules_config_for_guild') and callable(self._game_manager.get_core_rules_config_for_guild):
            rules_config = await self._game_manager.get_core_rules_config_for_guild(guild_id)
        ...
        try:
            if self.db_service:
                async with self.db_service.get_session() as session:
                    if base_action_type in self.state_changing_actions:
                        async with session.begin():
                            # Original logic for actions
                            if base_action_type == "MOVE":
                                ...
                                if target_entity_data:
                                    result = await self.handle_move_action(character, target_entity_data, guild_id, action_request.context_channel_id)
                                else:
                                ...
                                result = await self.handle_explore_action(character, guild_id, explore_params, action_request.context_channel_id)
                                ...
                                if skill_id_or_name:
                                    result = await self.handle_skill_use_action(character, skill_id_or_name, target_entity_data, action_data, guild_id, action_request.context_channel_id)
                                else:
                                    result = {"success": False, "message": "Какое умение ис��ользовать?", "state_changed": False}
                                ...
                                if item_entity_data:
                                    result = await self.handle_pickup_item_action(character, item_entity_data, guild_id, action_request.context_channel_id)
                                else:
                                    ...
                                    if target_entity_data and target_entity_data.get("id"):
                                        ...
                                        if target_obj_type in ["character", "player_character"] or (target_obj_type == "self" and target_obj_id == character.id):
                                            if target_obj_id == character.id or target_obj_type == "self":
                                                ...
                                                actual_target_entity_obj = self._character_manager.get_character(guild_id, target_obj_id)
                                        elif target_obj_type == "npc" and self._npc_manager:
                                            actual_target_entity_obj = self._npc_manager.get_npc(guild_id, target_obj_id)
                    ...
                    else: # Non-state changing actions, still within session but no explicit transaction
                        if base_action_type == "LOOK": # Example of a non-state changing action
                            explore_params = {'target': None}
                            look_target_entity = next((e for e in action_data.get("entities", []) if e.get("type") not in ["command", "intent"]), None)
                            if look_target_entity:
                                explore_params['target'] = look_target_entity.get("value", look_target_entity.get("name"))
                            ...
                            logger.warning(f"CAP.process_action_from_request: Unhandled non-state changing action type '{base_action_type}' for character {character.id}.")
                            result = {"success": False, "message": f"Действие '{base_action_type}' не реализовано.", "state_changed": False}

        except Exception as e:
            logger.error(f"CAP.process_action_from_request: Exception during {base_action_type} for {character.id}: {e}", exc_info=True)
            if base_action_type in self.state_changing_actions and session:
    ...
    async def is_busy(self, guild_id: str, character_id: str) -> bool:
         ...
         if party_id_val and self._party_manager and hasattr(self._party_manager, 'is_party_busy') and callable(self._party_manager.is_party_busy):
             return self._party_manager.is_party_busy(str(party_id_val), guild_id=str(char.guild_id))
         return False
    ...
    async def start_action(self, character_id: str, action_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        if await self.is_busy(guild_id, character_id):
        ...
        if action_type == 'move':
             ...
             if not target_location_id:
                  ...
                  await self._notify_character(guild_id, character_id, f"❌ Ошибка перем��щения: не указана целевая локация.")
    ...
    async def handle_move_action(self, character: Character, destination_entity: Dict[str, Any], guild_id: str, context_channel_id: Optional[int] = None) -> Dict[str, Any]:
        if not self._location_manager or not self._character_manager:
            return {"success": False, "message": "Movement failed: A required service is unavailable.", "state_changed": False}

        current_loc_id = character.current_location_id
        if not current_loc_id:
            return {"success": False, "message": "Movement failed: Current location unknown.", "state_changed": False}

        current_location = self._location_manager.get_location_instance(guild_id, current_loc_id)
        if not current_location:
            return {"success": False, "message": f"Movement failed: Could not find data for your current location (ID: {current_loc_id}).", "state_changed": False}

        destination_name = destination_entity.get("value") or destination_entity.get("name")
        if not destination_name:
            return {"success": False, "message": "Where do you want to go?", "state_changed": False}

        target_location_id = None
        if current_location.exits:
            for exit_data in current_location.exits:
                if exit_data.get('direction', '').lower() == destination_name.lower():
                    target_location_id = exit_data.get('target_location_id')
                    break
        
        if not target_location_id:
            return {"success": False, "message": f"You can't go '{destination_name}' from here.", "state_changed": False}

        target_location = self._location_manager.get_location_instance(guild_id, target_location_id)
        if not target_location:
            return {"success": False, "message": "The path leads to an unknown place.", "state_changed": False}

        await self._character_manager.update_character_location(character.id, target_location_id, guild_id)

        lang_code = character.selected_language or 'en'
        return {
            "success": True,
            "message": f"You move to **{target_location.name_i18n.get(lang_code, 'an unknown place')}**.",
            "state_changed": True
        }
    ...
    async def handle_explore_action(self, character: Character, guild_id: str, action_params: Dict[str, Any], context_channel_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            if not self._location_manager or not self._character_manager or not self._item_manager:
                logging.warning(f"CharacterActionProcessor.handle_explore_action: A required manager is not available.")
                return {'success': False, 'message': 'Exploration failed: A required service is unavailable.', 'data': {}}
            ...
            location = self._location_manager.get_location_instance(guild_id, current_loc_id)
            if not location:
                return {'success': False, 'message': f'Exploration failed: Could not find data for your current location (ID: {current_loc_id}).', 'data': {}}

            lang_code = character.selected_language or 'en'
            
            # Get characters in location
            characters_in_loc = await self._character_manager.get_characters_in_location(guild_id, current_loc_id)
            other_characters = [c for c in characters_in_loc if c.id != character.id]

            # Get items in location
            items_in_loc = await self._item_manager.get_items_in_location(guild_id, current_loc_id)

            # Get points of interest
            points_of_interest = location.points_of_interest_json or []

            # Format the message
            message_parts = [f"**{location.name_i18n.get(lang_code, location.name_i18n.get('en', 'Unknown Location'))}**"]
            message_parts.append(location.descriptions_i18n.get(lang_code, location.descriptions_i18n.get('en', 'No description.')))

            if other_characters:
                message_parts.append("\n**Characters:**")
                for char in other_characters:
                    message_parts.append(f"- {char.name}")

            if items_in_loc:
                message_parts.append("\n**Items on the ground:**")
                for item in items_in_loc:
                    message_parts.append(f"- {item.name_i18n.get(lang_code, item.name_i18n.get('en', 'Unknown Item'))}")

            if points_of_interest:
                message_parts.append("\n**Points of Interest:**")
                for poi in points_of_interest:
                    message_parts.append(f"- {poi.get('name_i18n', {}).get(lang_code, 'Unknown Point of Interest')}")

            if location.exits:
                message_parts.append("\n**Exits:**")
                for exit_data in location.exits:
                    message_parts.append(f"- {exit_data.get('direction', 'Unknown Direction')}")

            return {'success': True, 'message': "\n".join(message_parts), 'data': {}}
        except Exception as e:
        ...
        try:
            ...
            if self._game_log_manager and hasattr(self._game_log_manager, 'log_error') and callable(self._game_log_manager.log_error):
                 await self._game_log_manager.log_error(
                    ...
                    guild_id=guild_id, actor_id=actor_id, details={"traceback": "error logging traceback"}
                )
    ...
    async def handle_drop_item_action(self, character: Character, guild_id: str, action_data: Dict[str, Any], rules_config: "CoreGameRulesConfig") -> Dict[str, Any]:
        ...
        item_to_drop_data = self._inventory_manager.get_item_instance_by_id(guild_id, str(character.id), item_instance_id)
        ...
        if not added_to_loc_success:
            ...
            await self._inventory_manager.add_item(guild_id, str(character.id), item_template_id, int(item_quantity), item_data=dropped_item_copy)
            if self._character_manager and hasattr(self._character_manager, 'mark_character_dirty') and callable(self._character_manager.mark_character_dirty):

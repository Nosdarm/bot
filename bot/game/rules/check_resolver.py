...
from bot.database.models import Player, NPC
from bot.game.models.check_models import CheckResult
from bot.game.rules import dice_roller
...
class CheckResolver:
    def __init__(self, game_manager: "GameManager"):
        ...
        # logger.info("CheckResolver initialized.") # Optional: add logging if desired
    ...
    async def resolve_check(
        ...
        # game_manager is removed, use self.game_manager
        ...
        if performing_entity_type.lower() == "player":
            entity = await self.game_manager.get_player_model_by_id(guild_id, performing_entity_id)
            ...
            if self.game_manager.npc_manager:
                 entity = await self.game_manager.npc_manager.get_npc(guild_id, performing_entity_id)
                 if entity: entity_name_for_log = getattr(entity, 'name_i18n', {}).get('en', performing_entity_id)
            else:
        ...
        effective_stats = await calculate_effective_stats(entity, guild_id, self.game_manager)
        ...
        defined_skill = await self.game_manager.get_rule(guild_id, skill_rule_key)
        ...
        if defined_skill and isinstance(defined_skill, str):
            ...
            defined_attribute = await self.game_manager.get_rule(guild_id, attribute_rule_key)
        ...
        if difficulty_dc is not None:
            ...
            if target_entity_type.lower() == "player":
                target_entity = await self.game_manager.get_player_model_by_id(guild_id, target_entity_id)
                ...
                if self.game_manager.npc_manager:
                    target_entity = await self.game_manager.npc_manager.get_npc(guild_id, target_entity_id)
                    if target_entity: target_entity_name_for_log = getattr(target_entity, 'name_i18n', {}).get('en', target_entity_id)
                else:
            ...
            if not target_entity:
                ...
                target_effective_stats = await calculate_effective_stats(target_entity, guild_id, self.game_manager)

                opposed_by_stat_key = f"checks.{check_type}.opposed_by_stat"
                opposed_by_skill_key = f"checks.{check_type}.opposed_by_skill"

                defined_opposing_stat_name = await self.game_manager.get_rule(guild_id, opposed_by_stat_key)
                defined_opposing_skill_name = await self.game_manager.get_rule(guild_id, opposed_by_skill_key)

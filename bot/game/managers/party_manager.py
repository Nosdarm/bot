                 ...
                 for party in parties_to_save:
                      try:
                           ...
                           if leader_id is None:
                                print(f"PartyManager: Warning: Party {party_id} ('{getattr(party, 'name', party_id)}') (guild {guild_id_str}) has no leader_id during save prep.")

                           if not isinstance(member_ids_list, list):
                               print(f"PartyManager: Warning: Party {party_id} ('{getattr(party, 'name', party_id)}') (guild {guild_id_str}) player_ids_list is not a list during upsert prep ({type(member_ids_list)}). Saving as empty list.")
                               member_ids_list = []

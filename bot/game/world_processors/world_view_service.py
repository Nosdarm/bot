...
class WorldViewService:
    ...
    async def get_location_description(self,
        ...
        if viewer_entity_type == 'Character' and self._character_manager:
            char_result = self._character_manager.get_character_sync(guild_id, viewer_entity_id)
        ...
        if self._db_service:
            ...
            for key in relevant_world_states:
                raw_value = self._db_service.get_global_state_value_sync(key)
                if raw_value:
        ...
        location_data_result = self._location_manager.get_location_instance_sync(guild_id, location_id)
        ...
        if self._character_manager:
            all_characters_result = self._character_manager.get_characters_in_location_sync(guild_id, location_id)
        ...
        if self._npc_manager:
            all_npcs_result = self._npc_manager.get_npcs_in_location_sync(guild_id, location_id)
        ...
        if self._item_manager:
            items_in_loc_result = self._item_manager.get_items_by_owner_sync(guild_id, location_id, owner_type="location")
            for item_any in items_in_loc_result:
                item = cast(Item, item_any)
                entities_in_location.append(item)

        if self._party_manager:
            all_parties_result = self._party_manager.get_all_parties_for_guild_sync(guild_id)
        ...
        if entities_to_list:
            description_text += f"\n{get_i18n_text(None, 'you_see_here_label', player_lang, default_text='Вы видите здесь:', guild_id=guild_id)}\n"
            ...
            for entity in entities_to_list:
                ...
                entity_name = get_i18n_text(entity_name_i18n_dict if isinstance(entity_name_i18n_dict, dict) else None, 'name_i18n', player_lang, default_lang=DEFAULT_BOT_LANGUAGE, default_text=getattr(entity, 'name', 'Unknown Entity'), guild_id=guild_id)


                entity_type_display = "???"
                if isinstance(entity, Character): entity_type_display = get_i18n_text(None, "entity_type_character", player_lang, default_text="Персонаж", guild_id=guild_id)
                elif isinstance(entity, NPC): entity_type_display = get_i18n_text(None, "entity_type_npc", player_lang, default_text="NPC", guild_id=guild_id)
                elif isinstance(entity, Item): entity_type_display = get_i18n_text(None, "entity_type_item", player_lang, default_text="Предмет", guild_id=guild_id)
                elif isinstance(entity, Party): entity_type_display = get_i18n_text(None, "entity_type_party", player_lang, default_text="Партия", guild_id=guild_id)
                ...
                if isinstance(entity, NPC) and self._relationship_manager and viewer_char_obj:
                    ...
                    viewer_relationships_result = self._relationship_manager.get_relationships_for_entity_sync(guild_id, viewer_char_obj.id)
        ...
        if self._quest_manager and viewer_char_obj:
            active_quest_data_list_result = self._quest_manager.list_quests_for_character_sync(guild_id, viewer_char_obj.id)
            active_quest_data_list = active_quest_data_list_result if active_quest_data_list_result else []
            quest_status_text: str = ""
            if active_quest_data_list:
                active_quests_label = get_i18n_text(None, "active_quests_label", player_lang, default_text="Active Quests", guild_id=guild_id)
                ...
                for quest_data_item in active_quest_data_list:
                    ...
                    if current_stage_id:
                        stage_title = quest_obj.get_stage_title_sync(str(current_stage_id))
                        stage_desc = quest_obj.get_stage_description_sync(str(current_stage_id))
                    ...
                    if quest_name and "not found" not in quest_name.lower():
                        if "not found" in current_objective_desc.lower() or not current_objective_desc.strip(): current_objective_desc = get_i18n_text(None, "objective_not_specified_label", player_lang, default_text="Objective details not specified.", guild_id=guild_id)
        ...
        if exits_data:
            exits_label = get_i18n_text(None, 'exits_label', player_lang, default_text='Exits:', guild_id=guild_id)
            ...
            for exit_key in sorted_exit_keys:
                ...
                exit_display_name = get_i18n_text(exit_info, 'name_i18n', player_lang, default_lang=DEFAULT_BOT_LANGUAGE, default_text=exit_key.capitalize(), guild_id=guild_id)

                target_location_static_data_res = self._location_manager.get_location_instance_sync(guild_id, str(target_location_id_exit)) if target_location_id_exit else None
                ...
                if target_location_static_data:
                    target_location_name_display = get_i18n_text(target_location_static_data, 'name_i18n', player_lang, default_lang=DEFAULT_BOT_LANGUAGE, default_text=str(target_location_id_exit), guild_id=guild_id)
                description_text += f"- **{exit_display_name}** {get_i18n_text(None, 'leads_to_label', player_lang, default_text='ведет в', guild_id=guild_id)} '{target_location_name_display}'\n"
        else:
            description_text += f"\n{get_i18n_text(None, 'no_exits_label', player_lang, default_text='Выходов из этой локации нет.', guild_id=guild_id)}\n"
    ...
    async def get_entity_details(self,
        ...
        if viewer_entity_type == 'Character' and self._character_manager:
            char_res_details = self._character_manager.get_character_sync(guild_id, viewer_entity_id)
        ...
        if entity_type == 'Character' and self._character_manager:
             char_entity_res = self._character_manager.get_character_sync(guild_id, entity_id)
             ...
             npc_entity_res = self._npc_manager.get_npc_sync(guild_id, entity_id)
             ...
             item_entity_res = self._item_manager.get_item_instance_by_id_sync(guild_id, entity_id)
             ...
             party_entity_res = self._party_manager.get_party_sync(guild_id, entity_id)
        ...
        if isinstance(entity_name_i18n_val, dict):
             entity_name = get_i18n_text(entity_name_i18n_val, 'name_i18n', player_lang, default_lang=DEFAULT_BOT_LANGUAGE, default_text=entity_name_val, guild_id=guild_id)
        elif isinstance(entity_name_val, str):
            entity_name = entity_name_val
        else:
            entity_name = get_i18n_text(None, "unknown_entity_name", player_lang, default_text="Unknown Entity", guild_id=guild_id)


        entity_type_display_details = get_i18n_text(None, f"entity_type_{entity_type.lower()}", player_lang, default_text=entity_type, guild_id=guild_id)
        ...
        def _format_basic_entity_details_placeholder(entity_obj: Any, entity_type_str: str, lang: str, guild_id_for_i18n: str, observed_details_dict: Dict[str, Any]) -> str:
            details = ""
            type_label = get_i18n_text(None, "label_type", lang, default_text="Тип", guild_id=guild_id_for_i18n)
            name_label = get_i18n_text(None, "label_name", lang, default_text="Имя", guild_id=guild_id_for_i18n)
            health_label = get_i18n_text(None, "label_health", lang, default_text="Здоровье", guild_id=guild_id_for_i18n)
            alive_label = get_i18n_text(None, "label_alive", lang, default_text="Жив", guild_id=guild_id_for_i18n)
            yes_label = get_i18n_text(None, "label_yes", lang, default_text="Да", guild_id=guild_id_for_i18n)
            no_label = get_i18n_text(None, "label_no", lang, default_text="Нет", guild_id=guild_id_for_i18n)
            template_label = get_i18n_text(None, "label_template", lang, default_text="Шаблон", guild_id=guild_id_for_i18n)
            owner_id_label = get_i18n_text(None, "label_owner_id", lang, default_text="Владелец ID", guild_id=guild_id_for_i18n) # Corrected parameter name
            location_id_label = get_i18n_text(None, "label_location_id", lang, default_text="Локация ID", guild_id=guild_id_for_i18n) # Corrected parameter name
            leader_id_label = get_i18n_text(None, "label_leader_id", lang, default_text="Лидер ID", guild_id=guild_id_for_i18n) # Corrected parameter name
            members_label = get_i18n_text(None, "label_members", lang, default_text="Участники", guild_id=guild_id_for_i18n) # Corrected parameter name
            none_label = get_i18n_text(None, "label_none", lang, default_text="нет", guild_id=guild_id_for_i18n) # Corrected parameter name
            unknown_label = get_i18n_text(None, "label_unknown", lang, default_text="Неизвестно", guild_id=guild_id_for_i18n) # Corrected parameter name
            na_label = get_i18n_text(None, "label_na", lang, default_text="N/A", guild_id=guild_id_for_i18n) # Corrected parameter name
            ...
            entity_display_name_inner = get_i18n_text(entity_obj_dict.get('name_i18n') if isinstance(entity_obj_dict.get('name_i18n'), dict) else None, 'name_i18n', lang, default_lang=DEFAULT_BOT_LANGUAGE, default_text=entity_obj_dict.get('name', unknown_label), guild_id=guild_id_for_i18n)

            entity_type_display_placeholder_inner = get_i18n_text(None, f"entity_type_{entity_type_str.lower()}", lang, default_lang=DEFAULT_BOT_LANGUAGE, default_text=entity_type_str, guild_id=guild_id_for_i18n)
